...
[.lead]
vsa.azure events are reported when oNTAP(R) Cloud on Microsoft(R) Azure(R) receives a probe, indicating that a transition to the expected normal state has occurred, and corrective actions typically involve following the documented corrective action guidance.
