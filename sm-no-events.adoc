...
[.lead]
Learn about sm.no EMS events, which report when a SnapMirror(R) catalog metadata transfer is attempted and a SnapMirror object store configuration with "usage" of "metadata" is not configured on the system.
