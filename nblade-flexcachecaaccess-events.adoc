...
[.lead]
Learn about nblade.flexcachecaaccess EMS events, which report when a client attempts to access a FlexCache(R) volume via SMB3 using a continuously available share but is denied because FlexCache does not support persistent handle capability.
