...
[.lead]
nblade.flexcachecaaccess events are reported when a client tries to access a FlexCache(R) volume via SMB3 using a continuously available share but is denied because FlexCache does not support persistent handle capability, and corrective actions typically involve following the documented corrective action guidance.
