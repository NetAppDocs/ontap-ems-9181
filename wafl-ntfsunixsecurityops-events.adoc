...
[.lead]
Learn about wafl.ntfsunixsecurityops EMS events, which report when a UNIX-type permissions or ownership change is attempted by an NFS client on an NTFS (Windows) volume, but changes of this type are prohibited by an export policy rule.
