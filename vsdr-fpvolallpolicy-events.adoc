...
[.lead]
vsdr.fpvolallpolicy events are reported when snapMirror(R) transfer happens between the source volume of an SVM disaster recovery (SVMDR) relationship having a tiering policy of "all" residing on a FabricPool aggregate on the primary cluster and destination volume residing on a FabricPool aggregate at the secondary cluster, and corrective actions typically involve following the documented corrective action guidance.
