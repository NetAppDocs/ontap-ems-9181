...
[.lead]
wafl.fc events are reported when a FlexCache(R) origin node detects that its FlexCache volume has disconnected while a lock reclaim operation is in progress, and corrective actions typically involve verifying the reported configuration and health state.
