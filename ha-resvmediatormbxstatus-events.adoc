...
[.lead]
ha.resvmediatormbxstatus events are reported when the local node attempts to reserve a mailbox disk while trying to acquire the takeover lock as part of the takeover operation, and corrective actions typically involve following the documented corrective action guidance.
