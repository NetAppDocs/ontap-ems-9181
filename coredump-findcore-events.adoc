...
[.lead]
coredump.findcore events are reported when the core dump process detects an error while checking for an unsaved core in the dump device, and corrective actions typically involve following the documented corrective action guidance.
