...
[.lead]
hamsg.dskchkpopulateerr events are reported when a disk inventory check cannot be sent to the local node's high-availability (HA) partner because the disk check request could not be correctly constructed, and corrective actions typically involve following the documented corrective action guidance.
