...
[.lead]
Learn about ha.bdfu EMS events, which report when This message occurs during storage failover (SFO) giveback or aggregate relocation when the source node cannot reenable background disk firmware update (BDFU) on the destination node.
