...
[.lead]
extcache.configunsupported events are reported when the controller contains an external cache hardware configuration that cannot be supported on the controller, and corrective actions typically involve following the documented corrective action guidance.
