...
[.lead]
csm.ctfallbackswitchless events are reported when the Cluster Session Manager (CSM) defaults to use CT (TCP transport) when creating LIF pairs because remote device ID information is not available on switchless clusters running CSM over remote direct memory access (RDMA), and corrective actions typically involve following the documented corrective action guidance.
