...
[.lead]
pcp.linkstatechangesucceed events are reported when a change of network port state can now be confirmed after the initial attempt failed due to some race condition, and corrective actions typically involve following the documented corrective action guidance.
