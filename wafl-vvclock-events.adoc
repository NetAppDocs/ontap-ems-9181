...
[.lead]
Learn about wafl.vvclock EMS events, which report when an operation is run on an aggregate while the flexible volume level admin (vvc) lock of one of volumes of the aggregate is held by another subsystem.
