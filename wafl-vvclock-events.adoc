...
[.lead]
wafl.vvclock events are reported when an operation is run on an aggregate while the flexible volume level admin (vvc) lock of one of volumes of the aggregate is held by another subsystem, and corrective actions typically involve following the documented corrective action guidance.
