...
[.lead]
vsdr.qos events are reported when this message occurs during QoS configuration replication in a MetroCluster(tm) setup when there are maximum number of QoS policy groups already existing on the destination cluster and the system attempts to create a new QoS policy group, and corrective actions typically involve following the documented corrective action guidance.
