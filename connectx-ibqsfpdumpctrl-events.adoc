...
[.lead]
Learn about connectx.ibqsfpdumpctrl EMS events, which report when InfiniBand retimer programming fails due to a quad small form-factor pluggable (QSFP) transceiver register dump failure, causing the control registers of QSFP to be dumped.
