...
[.lead]
fsm.low events are reported when fpolicy Service Manager (FSM) fails to send outage, alert, handshake, cancel or volume operation notification to the FPolicy server due to memory allocation error, and corrective actions typically involve checking the reported configuration and connectivity.
