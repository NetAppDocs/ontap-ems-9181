...
[.lead]
nblade.nfsv4nsdbdomainmismatch events are reported when an NFSv4 client queries or updates the user or group attribute of a file object but the requested value does not match the '-v4-id-domain' option value of a virtual server's NFS service, and corrective actions typically involve following the documented corrective action guidance.
