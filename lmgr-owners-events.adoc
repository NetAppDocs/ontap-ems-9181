...
[.lead]
lmgr.owners events are reported when a lock manager owner object cannot be allocated due to reaching the limit for owner objects in the associated pool, and corrective actions typically involve following the documented corrective action guidance.
