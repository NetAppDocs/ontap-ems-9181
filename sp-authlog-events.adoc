...
[.lead]
sp.authlog events are reported when the Service Processor (SP) or baseboard management controller (BMC) fails to log an authentication event in the system event log (SEL), and corrective actions typically involve following the documented corrective action guidance.
