...
[.lead]
sfo.aggr events are reported when a storage failover aggregate is relocated from one node to another during a nondisruptive controller upgrade, and corrective actions typically involve following the documented corrective action guidance.
