...
[.lead]
nameserv.netgrpbyhost events are reported when netgroup is configured but the netgroup byhost database is not generated during an upgrade operation, and corrective actions typically involve following the documented corrective action guidance.
