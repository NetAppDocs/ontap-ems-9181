...
[.lead]
nblade.cifssametreepersessnotice events are reported when the number of connections to the same share allowed per session nears the configured limit, and corrective actions typically involve following the documented corrective action guidance.
