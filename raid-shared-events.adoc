...
[.lead]
raid.shared events are reported when a node receives an indication from its HA partner node that a shared disk is undergoing a change in state and the partner is waiting for the local node to complete a shared state transition, and corrective actions typically involve following the documented corrective action guidance.
