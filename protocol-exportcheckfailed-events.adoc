...
[.lead]
Learn about protocol.exportcheckfailed EMS events, which report when there is an export check failure due to, either export-policy configuration not allowing access, or a non-existing filesystem/junction access attempt.
