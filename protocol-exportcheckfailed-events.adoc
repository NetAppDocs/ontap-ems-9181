...
[.lead]
protocol.exportcheckfailed events are reported when there is an export check failure due to, either export-policy configuration not allowing access, or a non-existing filesystem/junction access attempt, and corrective actions typically involve checking the reported configuration and connectivity.
