...
[.lead]
sms.status events are reported when a SnapMirror(R) Sync initialize or resynchronize operation is completed successfully, and the source volume and destination volume are in "in-sync" status, and corrective actions typically involve following the documented corrective action guidance.
