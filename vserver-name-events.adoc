...
[.lead]
vserver.name events are reported when use of the "vserver rename" command creates a Vserver name conflict in the local cluster, and the local cluster resolves the name conflict by incorporating the Vserver Unique Identifier (UID) into the name, and no corrective action is typically required.
