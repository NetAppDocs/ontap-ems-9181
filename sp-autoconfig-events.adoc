...
[.lead]
sp.autoconfig events are reported when the system cannot automatically obtain IP configuration because the subnet associated with the Service Processor does not have any available IP addresses, and corrective actions typically involve following the documented corrective action guidance.
