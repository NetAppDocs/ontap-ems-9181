...
[.lead]
config.badpoolassign events are reported when syncMirror(R) is in use and disks are assigned to different pools on the same interface or loop, and corrective actions typically involve following the documented corrective action guidance.
