...
[.lead]
fci.adapter events are reported when the the Fibre Channel adapter driver is setting an option based on the setting of a bootarg variable, and corrective actions typically involve following the documented corrective action guidance.
