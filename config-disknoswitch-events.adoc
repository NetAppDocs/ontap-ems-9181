...
[.lead]
Learn about config.disknoswitch EMS events, which report when a disk is found to be attached to a switch, but it's not supported for a switch configuration.
