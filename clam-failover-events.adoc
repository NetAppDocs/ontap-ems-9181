...
[.lead]
clam.failover events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) receives a failover event from high-availability(HA) subsystem, and corrective actions typically involve following the documented corrective action guidance.
