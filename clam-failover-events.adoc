...
[.lead]
Learn about clam.failover EMS events, which report when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) receives a failover event from high-availability(HA) subsystem.
