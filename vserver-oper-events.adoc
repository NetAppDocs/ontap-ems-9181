...
[.lead]
vserver.oper events are reported when the system detects a cluster reboot on a Disaster Recovery Source (MetroCluster(tm) configuration or Vserver DR), and corrective actions typically involve following the documented corrective action guidance.
