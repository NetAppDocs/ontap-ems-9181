...
[.lead]
cpeer.ping events are reported when an attempt is made to check the cluster peering relationship and no intercluster LIFs of the same address family and the correct IPspace are found on this node, and corrective actions typically involve following the documented corrective action guidance.
