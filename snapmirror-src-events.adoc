...
[.lead]
snapmirror.src events are reported when this event is issued when a SnapMirror is issued from a source volume which is offline, restricted or does not exist, and corrective actions typically involve following the documented corrective action guidance.
