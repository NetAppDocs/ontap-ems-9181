...
[.lead]
sm.syncmirror events are reported when a sync granular CG relationship transitions from in-sync status to out-of-sync status, and corrective actions typically involve verifying the reported configuration and health state.
