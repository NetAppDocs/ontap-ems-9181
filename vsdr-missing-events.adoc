...
[.lead]
Learn about vsdr.missing EMS events, which report when the clone parent volume does not exist during the Vserver SnapMirror(R)/MetroCluster(tm) baseline replication on the secondary site.
