...
[.lead]
vsdr.missing events are reported when the clone parent volume does not exist during the Vserver SnapMirror(R)/MetroCluster(tm) baseline replication on the secondary site, and corrective actions typically involve following the documented corrective action guidance.
