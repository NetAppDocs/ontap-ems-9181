...
[.lead]
rdb.node events are reported when the replicated database (RDB) detects CPU starvation on this node, potentially causing it to leave cluster quorum, and corrective actions typically involve following the documented corrective action guidance.
