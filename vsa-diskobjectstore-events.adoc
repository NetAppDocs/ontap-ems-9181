...
[.lead]
Learn about vsa.diskobjectstore EMS events, which report when the system detects that a disk object store configuration is not synchronized between both nodes in a shared-storage High-Availability (HA) pair.
