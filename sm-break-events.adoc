...
[.lead]
sm.break events are reported when the "storage efficiency" feature is enabled on a destination volume due to a broken SnapMirror(R) relationship on an All Flash Optimized system, and corrective actions typically involve following the documented corrective action guidance.
