...
[.lead]
Learn about sm.break EMS events, which report when the "storage efficiency" feature is enabled on a destination volume due to a broken SnapMirror(R) relationship on an All Flash Optimized system.
