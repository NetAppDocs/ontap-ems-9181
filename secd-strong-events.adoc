...
[.lead]
Learn about secd.strong EMS events, which report when the external LDAP server configured for a Vserver is enforcing stronger authentication than what the ONTAP(R) LDAP client is configured for.
