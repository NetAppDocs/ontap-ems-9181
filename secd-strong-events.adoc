...
[.lead]
secd.strong events are reported when the external LDAP server configured for a Vserver is enforcing stronger authentication than what the ONTAP(R) LDAP client is configured for, and corrective actions typically involve following the documented corrective action guidance.
