...
[.lead]
nblade.notactivevscanserver events are reported when a connection from a remote IP is not found in the Vscan scanner pool belonging to the storage virtual machine (SVM), and corrective actions typically involve following the documented corrective action guidance.
