...
[.lead]
Learn about raid.flexvol EMS events, which report when a configuration operation on an aggregate (for example, adding disks, relocating aggregate disks, etc.), fails to get exclusive lock access due to one or more indirect conflict volume locks held by child volumes within the aggregate.
