...
[.lead]
raid.aggr events are reported when the usable size of an aggregate does not expand after a corresponding increase in the size of the underlying storage, and corrective actions typically involve checking the reported configuration and connectivity.
