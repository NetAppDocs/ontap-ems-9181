...
[.lead]
raid.multierr events are reported when rAID encounters more errors than the RAID level protection affords, and RAID cannot recover the block, and corrective actions typically involve checking the reported configuration and connectivity.
