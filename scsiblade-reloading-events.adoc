...
[.lead]
scsiblade.reloading events are reported when the local node is recovering from a connectivity loss by purging and reloading the SAN configuration cache, and corrective actions typically involve following the documented corrective action guidance.
