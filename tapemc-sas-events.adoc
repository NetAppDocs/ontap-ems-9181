...
[.lead]
tapemc.sas events are reported when data ONTAP(R) detects a SAS device that is connected to a port that is dedicated for SAS disk storage attachment, but is not a disk shelf or a bridge device, and corrective actions typically involve following the documented corrective action guidance.
