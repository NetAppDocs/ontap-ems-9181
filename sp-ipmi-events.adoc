...
[.lead]
Learn about sp.ipmi EMS events, which report when Data ONTAP(R) is no longer receiving the heartbeats from the Service Processor (SP) after all the SP recovery actions have been taken.
