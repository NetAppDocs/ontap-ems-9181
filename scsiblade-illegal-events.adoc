...
[.lead]
Learn about scsiblade.illegal EMS events, which report when an error is returned for a SCSI command that exceeds the LUN's maximum supported transfer length of 1MB.
