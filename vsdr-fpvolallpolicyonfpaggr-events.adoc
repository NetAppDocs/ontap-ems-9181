...
[.lead]
Learn about vsdr.fpvolallpolicyonfpaggr EMS events, which report when source volume of an SVM disaster recovery (SVMDR) relationship having a volume tiering policy of "all" residing on FabricPool aggregate on the primary cluster is placed on a FabricPool aggregate at the destination cluster.
