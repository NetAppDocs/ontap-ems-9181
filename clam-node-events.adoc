...
[.lead]
clam.node events are reported when the local Connectivity, Liveliness and Availability Monitor (CLAM) concludes that the local node is now disabled, and corrective actions typically involve following the documented corrective action guidance.
