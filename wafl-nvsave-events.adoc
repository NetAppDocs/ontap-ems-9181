...
[.lead]
wafl.nvsave events are reported when one or more NVSave files are deleted from the local root volume or the partner root volume while in takeover mode, or from the partner node when the partner node is not in takeover mode, and corrective actions typically involve following the documented corrective action guidance.
