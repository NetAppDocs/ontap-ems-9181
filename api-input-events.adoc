...
[.lead]
api.input events are reported when an ONTAPI(tm) API receives extra input parameters as part of the API request or when a required input parameter is missing in the API request, and corrective actions typically involve following the documented corrective action guidance.
