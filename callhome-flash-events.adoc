...
[.lead]
callhome.flash events are reported when flash Management Module (FMM) detects that a percentage of a Flash Cache is offline that is above a warning threshold, and corrective actions typically involve following the documented corrective action guidance.
