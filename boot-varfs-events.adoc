...
[.lead]
boot.varfs events are reported when an error is detected while trying to back up the /var file system to the boot media or node root volume (mroot), and corrective actions typically involve contacting NetApp technical support.
