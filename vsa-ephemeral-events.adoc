...
[.lead]
vsa.ephemeral events are reported when a node is configured to use ephemeral vNVRAM and the virtual machine has been re-hosted, using new ephemeral disk for vNVRAM, and corrective actions typically involve following the documented corrective action guidance.
