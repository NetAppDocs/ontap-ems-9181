...
[.lead]
vserver.volume events are reported when a volume is placed on nonhomogeneous aggregates, because of errors on homogeneous aggregates, such as max volumes is reached or aggregate is not home, and corrective actions typically involve following the documented corrective action guidance.
