...
[.lead]
vsdr.ip events are reported when a logical interface (LIF) is automatically created through configuration replication in a MetroCluster(TM) configuration, and corrective actions typically involve following the documented corrective action guidance.
