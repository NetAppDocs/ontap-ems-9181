...
[.lead]
fci.config events are reported when the system reboots automatically to enable correct adapter operation after changing the state of a reconfigurable adapter, and corrective actions typically involve following the documented corrective action guidance.
