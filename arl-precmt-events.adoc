...
[.lead]
arl.precmt events are reported when this message occurs during aggregate relocation, when automatic lock resynchronization with the node's high-availability (HA) partner for files on this aggregate was aborted, and corrective actions typically involve following the documented corrective action guidance.
