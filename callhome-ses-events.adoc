...
[.lead]
Learn about callhome.ses EMS events, which report when the storage adapter detects a disk on the Fibre Channel loop or the SCSI bus, but enclosure services in the disk shelf does not report a disk with that address.
