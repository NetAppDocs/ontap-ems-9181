...
[.lead]
usbmon.boot events are reported when the Data ONTAP(R) module that is responsible for monitoring the health of the Universal Serial Bus (USB) boot devices determines that the associated boot device will fail all writes to the media, and corrective actions typically involve following the documented corrective action guidance.
