...
[.lead]
cf.misc events are reported when the message occurs when a giveback is initiated before the partner has reached waiting-for-giveback state, and corrective actions typically involve following the documented corrective action guidance.
