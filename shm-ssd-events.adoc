...
[.lead]
shm.ssd events are reported when oNTAP(R) detects that a solid-state drive (SSD) has an average block latency during a five-minute timeframe that is greater than expected, and corrective actions typically involve following the documented corrective action guidance.
