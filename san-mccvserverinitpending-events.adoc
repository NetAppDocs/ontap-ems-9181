...
[.lead]
Learn about san.mccvserverinitpending EMS events, which report when This message occurs in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems when SAN Vservers exceed the time allotted for initialization during a switchover or switchback operation.
