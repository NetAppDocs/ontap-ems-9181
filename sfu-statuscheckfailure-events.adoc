...
[.lead]
Learn about sfu.statuscheckfailure EMS events, which report when the (advanced privilege) "storage shelf firmware update" command encounters a failure while attempting to read the status of the firmware update in progress.
