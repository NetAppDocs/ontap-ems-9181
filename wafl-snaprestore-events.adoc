...
[.lead]
wafl.snaprestore events are reported when this message occurs during a snapshot restore operation, when the system cannot recognize the version of Data ONTAP(R) from the snapshot it is reverting to, and corrective actions typically involve following the documented corrective action guidance.
