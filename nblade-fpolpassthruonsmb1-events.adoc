...
[.lead]
nblade.fpolpassthruonsmb1 events are reported when this message occurs during a client attempt to establish an FPolicy pass-through read channel over SMB1, which is not supported, and corrective actions typically involve verifying the reported configuration and health state.
