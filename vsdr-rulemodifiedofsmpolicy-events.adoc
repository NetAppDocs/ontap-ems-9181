...
[.lead]
Learn about vsdr.rulemodifiedofsmpolicy EMS events, which report when a rule attached to a SnapMirror(R) policy with "mirror-vault" type must be modified, so that L2 scheduled snapshots can be reused during a MAX Data schedule update.
