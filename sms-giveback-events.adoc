...
[.lead]
sms.giveback events are reported when this message occurs during an SFO aggregate giveback, when the SnapMirror(R) subsystem indicates that giveback should be vetoed but the veto was overridden by the automated nondisruptive update procedure, and corrective actions typically involve following the documented corrective action guidance.
