...
[.lead]
Learn about sms.giveback EMS events, which report when This message occurs during an SFO aggregate giveback, when the SnapMirror(R) subsystem indicates that giveback should be vetoed but the veto was overridden by the automated nondisruptive update procedure.
