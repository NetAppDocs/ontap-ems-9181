...
[.lead]
san.nonetworkportadaptertype events are reported when this message occurs in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP cannot find a port on the disaster recovery (DR) node that is of the same adapter type (converged network adapter (CNA)/network interface card (NIC)) as the home port of the primary storage virtual machine (SVM) data LIF, and corrective actions typically involve ensuring the required configuration and resources are in place.
