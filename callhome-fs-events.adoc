...
[.lead]
callhome.fs events are reported when the system has detected that a filesystem logical disk array or LUN (Logical Unit Number) stored raw size no longer matches the reported physical size of the LUN, and corrective actions typically involve following the documented corrective action guidance.
