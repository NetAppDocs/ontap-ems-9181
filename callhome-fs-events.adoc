...
[.lead]
Learn about callhome.fs EMS events, which report when the system has detected that a filesystem logical disk array or LUN (Logical Unit Number) stored raw size no longer matches the reported physical size of the LUN.
