...
[.lead]
sms.qr events are reported when wafl fails to allocate required memory for storing streams attributes information or asked buffer memory length is too less to hold all the streams attributes info, in both the cases the status of Snapmirror Synchronous relationship changes to "OutOfSync", and corrective actions typically involve following the documented corrective action guidance.
