...
[.lead]
acp.invalid events are reported when the Alternate Control Path (ACP) system detects that an Alternate Control Path Processor (ACPP) has invalid VPD (Vital Product Data) info, and corrective actions typically involve following the documented corrective action guidance.
