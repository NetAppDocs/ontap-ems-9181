...
[.lead]
cloud.creds events are reported when the system cannot establish a communication link with the cloud provider's metadata server, in order to acquire the necessary temporary credentials used in signing API requests to the cloud object store server, and corrective actions typically involve following the documented corrective action guidance.
