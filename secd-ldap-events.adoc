...
[.lead]
secd.ldap events are reported when the -bind-dn user is not configured and the -bind-as-cifs-server option is set to false in the Lightweight Directory Access Protocol (LDAP) client configuration, and corrective actions typically involve following the documented corrective action guidance.
