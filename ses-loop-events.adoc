...
[.lead]
ses.loop events are reported when the system finds a disk shelf with a Loop Resiliency Circuit (LRC) module before a disk shelf with an Electronically Switched Hub (ESH) module in the loop map, and corrective actions typically involve following the documented corrective action guidance.
