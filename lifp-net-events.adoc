...
[.lead]
Learn about lifp.net EMS events, which report when This message occurs while assigning network ports to broadcast domains, when LIF Placement subnet discovery finds a port that is assigned to a broadcast domain that is different from the broadcast domain to which other ports in the same physical broadcast domain are assigned.
