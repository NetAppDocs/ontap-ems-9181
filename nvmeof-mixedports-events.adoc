...
[.lead]
nvmeof.mixedports events are reported when there is no longer a nonrecommended mix of a dedicated port and a nondedicated port cabled from a node to a shelf, and corrective actions typically involve following the documented corrective action guidance.
