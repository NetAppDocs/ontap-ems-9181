...
[.lead]
Learn about wafl.afs EMS events, which report when the deswizzle scanner fails to enable the wafl.deswizzle.afs.first feature on the aggregate because block reallocation redirection cannot be enabled on the aggregate.
