...
[.lead]
Learn about splog.running EMS events, which report when the Data ONTAP(R) process (splogd) that monitors the Service Processor (SP) for available SP logs starts successfully or when it returns to normal operation from degraded mode.
