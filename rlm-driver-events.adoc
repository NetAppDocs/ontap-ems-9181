...
[.lead]
rlm.driver events are reported when the system encounters an error while trying to get hourly statistics from the Remote LAN Module (RLM), and corrective actions typically involve following the documented corrective action guidance.
