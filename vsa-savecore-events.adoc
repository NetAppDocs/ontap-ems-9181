...
[.lead]
Learn about vsa.savecore EMS events, which report when the savecore tool determines that the space available is not sufficient for storing the core image from a dump device to a readable core file in the /etc/crash directory.
