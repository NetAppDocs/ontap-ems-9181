...
[.lead]
wafl.aggr events are reported when an aggregate is taken offline due to the aggregate has savings because of aggregate deduplication and this version of ONTAP does not support aggregate level deduplication, and corrective actions typically involve following the documented corrective action guidance.
