...
[.lead]
acp.downrev events are reported when the Alternate Control Path Administrator cannot successfully download up-to-date firmware to an Alternate Control Path Processor (ACPP) or when the most up-to-date firmware image is not present in the /etc/acpp_fw/ directory, and corrective actions typically involve verifying the reported configuration and health state.
