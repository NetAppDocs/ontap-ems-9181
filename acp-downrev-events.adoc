...
[.lead]
Learn about acp.downrev EMS events, which report when the Alternate Control Path Administrator cannot successfully download up-to-date firmware to an Alternate Control Path Processor (ACPP) or when the most up-to-date firmware image is not present in the /etc/acpp_fw/ directory.
