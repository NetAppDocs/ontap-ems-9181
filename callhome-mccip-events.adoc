...
[.lead]
callhome.mccip events are reported when automatic unplanned switchover (AUSO) in a MetroCluster(tm) IP configuration is disabled because the mediator mailbox disk is not healthy, and corrective actions typically involve following the documented corrective action guidance.
