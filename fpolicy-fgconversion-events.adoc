...
[.lead]
fpolicy.fgconversion events are reported when a FlexVol(R) unmount notification is not sent to the FPolicy(tm) server during a FlexGroup conversion, and corrective actions typically involve following the documented corrective action guidance.
