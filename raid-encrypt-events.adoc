...
[.lead]
raid.encrypt events are reported when a node is enabled for encryption by RAID, but the system is not FIPS compliant, and corrective actions typically involve rebooting the node or cluster component.
