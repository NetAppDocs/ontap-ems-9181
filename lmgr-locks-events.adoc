...
[.lead]
lmgr.locks events are reported when a lock manager lock object cannot be allocated due to reaching the limit for lock objects in the associated pool, and corrective actions typically involve following the documented corrective action guidance.
