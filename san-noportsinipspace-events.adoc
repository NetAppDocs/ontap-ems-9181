...
[.lead]
san.noportsinipspace events are reported when an attempt to place an IP LIF on a particular node fails because the system cannot find any ports in an IPspace on that node, and corrective actions typically involve following the documented corrective action guidance.
