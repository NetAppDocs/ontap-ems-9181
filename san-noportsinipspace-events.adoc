...
[.lead]
Learn about san.noportsinipspace EMS events, which report when an attempt to place an IP LIF on a particular node fails because the system cannot find any ports in an IPspace on that node.
