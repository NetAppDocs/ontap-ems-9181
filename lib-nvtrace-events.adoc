...
[.lead]
Learn about lib.nvtrace EMS events, which report when the system sorts the syslog messages when syslog is not yet initialized, and the panic messages are relogged.
