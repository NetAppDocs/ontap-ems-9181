...
[.lead]
vifmgr.migratelifs events are reported when the network load balancing listener is not responding to probes over a node management LIF, and cluster-scoped LIFs with the same current port have been migrated elsewhere, and corrective actions typically involve following the documented corrective action guidance.
