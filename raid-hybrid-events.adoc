...
[.lead]
raid.hybrid events are reported when the sum of all sizes of solid state disks (SSDs) disks of hybrid aggregates on this node and its partner node is larger than the maximum system limit, and corrective actions typically involve following the documented corrective action guidance.
