...
[.lead]
disk.encryptsupportreqd events are reported when oNTAP(R) software detects one or more nonencrypting drives or non-FIPS-certified self-encrypting drives (SEDs), but the environment variable 'bootarg, and corrective actions typically involve following the documented corrective action guidance.
