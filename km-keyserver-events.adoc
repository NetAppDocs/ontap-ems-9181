...
[.lead]
km.keyserver events are reported when a check by the key manager for connectivity with a configured key management server indicates that the key management server that was previously unavailable is now available, and corrective actions typically involve following the documented corrective action guidance.
