...
[.lead]
rdb.internal events are reported when the cluster management Data Replication Module encounters an internal error, possibly causing the process to exit, and corrective actions typically involve verifying the reported configuration and health state.
