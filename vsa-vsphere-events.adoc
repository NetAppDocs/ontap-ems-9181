...
[.lead]
vsa.vsphere events are reported when data ONTAP(R) on VMware(R) manages to reconnect and authenticate itself with the vSphere(tm) server, and no corrective action is typically required.
