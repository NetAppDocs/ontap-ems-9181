...
[.lead]
Learn about csm.ctfallbackactiveopen EMS events, which report when the Cluster Session Manager (CSM) uses CT (TCP transport) as the default when creating LIF pairs because connections running CSM over remote direct memory access (RDMA) repeatedly fail during successive retry attempts.
