...
[.lead]
csm.ctfallbackactiveopen events are reported when the Cluster Session Manager (CSM) uses CT (TCP transport) as the default when creating LIF pairs because connections running CSM over remote direct memory access (RDMA) repeatedly fail during successive retry attempts, and corrective actions typically involve following the documented corrective action guidance.
