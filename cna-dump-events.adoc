...
[.lead]
cna.dump events are reported when an unexpected event or illegal condition is detected by the Converged Network Adapter (CNA) driver and the contents of the adapter's static RAM and memory must be dumped, and corrective actions typically involve following the documented corrective action guidance.
