...
[.lead]
Learn about callhome.rnode EMS events, which report when This messages occurs when a new set of log files has been collected from a Service Processor (SP) of one or more non-operational nodes in the cluster.
