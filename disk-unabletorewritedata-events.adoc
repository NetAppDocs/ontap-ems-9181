...
[.lead]
Learn about disk.unabletorewritedata EMS events, which report when Data ONTAP(R) receives too many errors reading from a particular location on an ATA disk drive, but its attempt to rewrite the data to the disk drive fails.
