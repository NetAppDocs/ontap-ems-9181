...
[.lead]
disk.unabletorewritedata events are reported when data ONTAP(R) receives too many errors reading from a particular location on an ATA disk drive, but its attempt to rewrite the data to the disk drive fails, and corrective actions typically involve following the documented corrective action guidance.
