...
[.lead]
quota.scan events are reported when this message occurs after the administrator uses the "volume quota on" command, when the quota scanner aborts without finishing due to an error condition, and corrective actions typically involve following the documented corrective action guidance.
