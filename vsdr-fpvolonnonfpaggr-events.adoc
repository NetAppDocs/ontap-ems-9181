...
[.lead]
vsdr.fpvolonnonfpaggr events are reported when a source volume of an SVM disaster recovery (SVMDR) relationship residing on a FabricPool aggregate at the primary cluster is placed on a non-FabricPool aggregate at the secondary cluster, and corrective actions typically involve following the documented corrective action guidance.
