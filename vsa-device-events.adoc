...
[.lead]
Learn about vsa.device EMS events, which report when the system detects that the kernel environment variables (kenvs) that are used to configure the core dump and NVRAM devices are missing or inconsistent.
