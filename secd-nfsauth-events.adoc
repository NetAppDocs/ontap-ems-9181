...
[.lead]
secd.nfsauth events are reported when an NFS authorization attempt fails because of the inability of the system to retrieve a matching CIFS credential for use in multi-protocol security operations, and corrective actions typically involve following the documented corrective action guidance.
