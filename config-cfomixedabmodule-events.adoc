...
[.lead]
Learn about config.cfomixedabmodule EMS events, which report when a disk shelf attached to a node is miscabled so that one disk shelf's module A is connected to the next disk shelf's module B, or vice versa.
