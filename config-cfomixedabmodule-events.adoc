...
[.lead]
config.cfomixedabmodule events are reported when a disk shelf attached to a node is miscabled so that one disk shelf's module A is connected to the next disk shelf's module B, or vice versa, and corrective actions typically involve following the documented corrective action guidance.
