...
[.lead]
cf.cdo events are reported when this event is issued when the filer receives a disk operation message over the High Availability (HA) interconnect that it does not recognize, and corrective actions typically involve following the documented corrective action guidance.
