...
[.lead]
qos.bgtask events are reported when the Quality of Service (QoS) subsystem starts throttling requests to help critical background tasks meet their min-throughput and their service-level objectives (SLOs), and corrective actions typically involve following the documented corrective action guidance.
