...
[.lead]
lmgr.delegations events are reported when a lock manager WAN FlexCache(R) delegation object cannot be allocated due to reaching the limit for WAN FlexCache delegation objects in the associated pool, and corrective actions typically involve following the documented corrective action guidance.
