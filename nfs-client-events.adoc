...
[.lead]
Learn about nfs.client EMS events, which report when an NFS connected client's cache settings are changed using the REST API "/api/protocol/nfs/connected-client-settings", but the value is not updated in the network component.
