...
[.lead]
nfs.client events are reported when an NFS connected client's cache settings are changed using the REST API "/api/protocol/nfs/connected-client-settings", but the value is not updated in the network component, and corrective actions typically involve following the documented corrective action guidance.
