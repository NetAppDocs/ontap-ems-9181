...
[.lead]
vf.cleanup events are reported when some subsystems are unable to cleanup properly when a local host vfiler is destroyed or when a partner vfiler is being shutdown during giveback, and corrective actions typically involve following the documented corrective action guidance.
