...
[.lead]
oauth2.localroles events are reported when an authentication fails when the "use-local-roles-if-present" setting is set to "true", but the user information does not exist in ONTAP, or the scope information within the access token does not grant sufficient access in ONTAP, and corrective actions typically involve following the documented corrective action guidance.
