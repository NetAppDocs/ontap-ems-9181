...
[.lead]
vifmgr.skippingl2pingtest events are reported when the number of local ports in the given broadcast domain exceeds the threshold for a node and, as a result, the l2_reachability health monitor has omitted checking ports in that broadcast domain, and corrective actions typically involve following the documented corrective action guidance.
