...
[.lead]
Learn about vifmgr.skippingl2pingtest EMS events, which report when the number of local ports in the given broadcast domain exceeds the threshold for a node and, as a result, the l2_reachability health monitor has omitted checking ports in that broadcast domain.
