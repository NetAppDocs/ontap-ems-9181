...
[.lead]
diskown.diskwasstolen events are reported when this event is issued when the ownership of a disk owned by the system is changed such that the disk is no longer owned by the system, and corrective actions typically involve following the documented corrective action guidance.
