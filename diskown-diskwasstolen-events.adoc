...
[.lead]
Learn about diskown.diskwasstolen EMS events, which report when This event is issued when the ownership of a disk owned by the system is changed such that the disk is no longer owned by the system.
