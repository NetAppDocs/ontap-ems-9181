...
[.lead]
zsm.socket events are reported when the event posted when the listener socket for the ZAPI Socket Manager (ZSM) service cannot be created, and corrective actions typically involve following the documented corrective action guidance.
