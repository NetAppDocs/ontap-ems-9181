...
[.lead]
Learn about sp.autoupd EMS events, which report when a Service Processor (SP) or baseboard management controller (BMC) firmware automatic update is cancelled because the high-availability (HA) partner node is offline.
