...
[.lead]
Learn about lmgr.move EMS events, which report when a volume move operation fails the cutover phase, due to volume quiesce timer expiration, and not all locks were transferred before the failure.
