...
[.lead]
wafl.i2p events are reported when the ONTAP(R) kernel detects that an i2p metafile on one of the snapshot has more entries than necessary for an inode, and corrective actions typically involve following the documented corrective action guidance.
