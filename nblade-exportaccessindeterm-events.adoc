...
[.lead]
nblade.exportaccessindeterm events are reported when client access cannot be evaluated because of an error while matching the client against export rules, and corrective actions typically involve verifying the reported configuration and health state.
