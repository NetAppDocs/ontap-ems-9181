...
[.lead]
nblade.s3bucketpathisstale events are reported when the path of the NAS directory corresponding to an S3 NAS Bucket is stale (deleted or deleted and recreated) after the bucket was created, and corrective actions typically involve checking the reported configuration and connectivity.
