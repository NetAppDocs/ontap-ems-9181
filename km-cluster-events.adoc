...
[.lead]
km.cluster events are reported when this message occurs in a MetroCluster(tm) configuration, when one or both clusters have been configured with the Onboard Key Manager, but one or both of the Onboard Key Manager key hierarchies are corrupted, and corrective actions typically involve rebooting the node or cluster component.
