...
[.lead]
callhome.client events are reported when a remote client accesses the node using Manage ONTAP(R) and triggers an event using the ems-autosupport-log API call with "auto-support" field set to "true", and corrective actions typically involve following the documented corrective action guidance.
