...
[.lead]
callhome.chassis events are reported when the chassis temperature is too warm and might cause an environmental problem, or when there is faulty hardware in the system, such as a fan failure or a malfunctioning temperature sensor, and corrective actions typically involve following the documented corrective action guidance.
