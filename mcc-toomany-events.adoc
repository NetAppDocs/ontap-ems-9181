...
[.lead]
mcc.toomany events are reported when a volume is added to an aggregate in which the recommended maximum number of volumes for an aggregate in a MetroCluster setup is exceeded, and corrective actions typically involve following the documented corrective action guidance.
