...
[.lead]
ha.takeoverimpunsync events are reported when the partner node is down and it is currently impossible to perform a takeover because the takeover node does not have partner NVRAM data, and corrective actions typically involve verifying the reported configuration and health state.
