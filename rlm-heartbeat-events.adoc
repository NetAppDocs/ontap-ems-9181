...
[.lead]
rlm.heartbeat events are reported when the appliance detects resumption of Remote LAN Module (RLM) heartbeat notifications indicating that the RLM is now available, and corrective actions typically involve following the documented corrective action guidance.
