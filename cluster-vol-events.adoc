...
[.lead]
cluster.vol events are reported when the volume required to store the system data needed for cluster-wide storage is created successfully after one or more attempts to create it failed, and corrective actions typically involve following the documented corrective action guidance.
