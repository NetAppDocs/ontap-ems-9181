...
[.lead]
token.vol events are reported when a 'volume offline' operation results in clearing the token cache for that volume, and corrective actions typically involve following the documented corrective action guidance.
