...
[.lead]
arl.veto events are reported when the coredump subsystem vetoes the use of storage failover (SFO) during takeover because a coredump save is in progress; cluster failover (CFO) will be used instead, and corrective actions typically involve following the documented corrective action guidance.
