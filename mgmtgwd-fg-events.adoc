...
[.lead]
mgmtgwd.fg events are reported when the user creates a FlexGroup on a Vserver that has NFSv3 enabled, but uses 32-bit identifiers, which could cause file ID collisions, and corrective actions typically involve following the documented corrective action guidance.
