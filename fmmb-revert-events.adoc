...
[.lead]
fmmb.revert events are reported when this event is issued when the HA Mailbox opaque recieves a write request for the opaque area and a revert_to command is in progress, and corrective actions typically involve following the documented corrective action guidance.
