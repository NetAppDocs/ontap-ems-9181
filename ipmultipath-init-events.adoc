...
[.lead]
ipmultipath.init events are reported when this message occurs during IP Multipath session initialization, when an INIT packet with an unknown checksum type is received, and corrective actions typically involve following the documented corrective action guidance.
