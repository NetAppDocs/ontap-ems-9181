...
[.lead]
bcomd.igroup events are reported when an initiator group marked with the delete-on-unmap flag is not automatically deleted when it is no longer part of a LUN mapping relationship, and corrective actions typically involve following the documented corrective action guidance.
