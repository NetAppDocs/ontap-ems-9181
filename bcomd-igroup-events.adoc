...
[.lead]
Learn about bcomd.igroup EMS events, which report when an initiator group marked with the delete-on-unmap flag is not automatically deleted when it is no longer part of a LUN mapping relationship.
