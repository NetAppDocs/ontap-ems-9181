...
[.lead]
nblade.fcvoldisconnected events are reported when the NFS client attempts to access a FlexCache(R) volume, but the origin of the FlexCache volume cannot be accessed due to poor network connectivity, and corrective actions typically involve verifying the reported configuration and health state.
