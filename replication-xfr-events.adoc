...
[.lead]
replication.xfr events are reported when a SnapMirror(R)/SnapVault(tm) operation is not completed because a block received from the source system failed checksum verification, and corrective actions typically involve following the documented corrective action guidance.
