...
[.lead]
lmgr.mismatch events are reported when the current file object of a lock does not match the file object found through the file handle, and no corrective action is typically required.
