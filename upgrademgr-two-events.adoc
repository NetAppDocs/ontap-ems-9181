...
[.lead]
upgrademgr.two events are reported when a two-stage version update starts on the cluster where the target release is two versions later than the current version, and corrective actions typically involve following the documented corrective action guidance.
