...
[.lead]
Learn about upgrademgr.two EMS events, which report when a two-stage version update starts on the cluster where the target release is two versions later than the current version.
