...
[.lead]
vsdr.volmove events are reported when a volume at the source of a Vserver-DR setup is encrypted, but the volume at the destination is not configured to be encrypted, and corrective actions typically involve verifying the reported configuration and health state.
