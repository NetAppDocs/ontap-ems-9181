...
[.lead]
Learn about vsdr.volmove EMS events, which report when a volume at the source of a Vserver-DR setup is encrypted, but the volume at the destination is not configured to be encrypted.
