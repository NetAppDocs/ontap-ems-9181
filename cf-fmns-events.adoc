...
[.lead]
Learn about cf.fmns EMS events, which report when a node is in "Waiting For Giveback" state and the Failover Monitor Node State (fmns)module skips a local disk while releasing the reservations in that state.
