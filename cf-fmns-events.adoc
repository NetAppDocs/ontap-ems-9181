...
[.lead]
cf.fmns events are reported when a node is in "Waiting For Giveback" state and the Failover Monitor Node State (fmns)module skips a local disk while releasing the reservations in that state, and corrective actions typically involve following the documented corrective action guidance.
