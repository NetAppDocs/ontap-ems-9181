...
[.lead]
wafl.spcmgmt events are reported when a space management strategy or an autosize grow threshold for a volume is being configured and the autogrow or autodelete thresholds conflict with the space-mgmt-try-first policy, and corrective actions typically involve following the documented corrective action guidance.
