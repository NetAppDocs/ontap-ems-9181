...
[.lead]
clam.start events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) has started a poll to read CLAM mailboxes, and corrective actions typically involve following the documented corrective action guidance.
