...
[.lead]
nameserv.ddns events are reported when a request to delete the fully qualified domain name (FQDN) of all or some of the logical interfaces of a Vserver from the DNS server fails, resulting in stale records on the DNS server, and corrective actions typically involve following the documented corrective action guidance.
