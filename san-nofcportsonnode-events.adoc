...
[.lead]
san.nofcportsonnode events are reported when this message occurs in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems when an attempt to place an FCP LIF on a particular node fails because the system cannot find any active ports on that node, and corrective actions typically involve following the documented corrective action guidance.
