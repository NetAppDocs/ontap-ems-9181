...
[.lead]
Learn about san.nofcportsonnode EMS events, which report when This message occurs in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems when an attempt to place an FCP LIF on a particular node fails because the system cannot find any active ports on that node.
