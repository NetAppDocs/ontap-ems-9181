...
[.lead]
raid.replay events are reported when this event is issued when an aggregate becomes available and system successfully replayed the preserved NVRAM entries associated with that aggregate, and corrective actions typically involve following the documented corrective action guidance.
