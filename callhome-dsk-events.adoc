...
[.lead]
callhome.dsk events are reported when a disk drive failure occurred and the Embedded Switched Hub (ESH) removed the disk drive from the loop electronically--bypassing the disk drive on one or possibly both ports, and corrective actions typically involve following the documented corrective action guidance.
