...
[.lead]
Learn about mgmtgwd.wafl EMS events, which report when the system will automatically enter RDB recovery because the system has detected that a root volume might have changed, possibly causing the local management databases to be out of sync with the replicated databases.
