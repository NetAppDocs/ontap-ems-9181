...
[.lead]
nodewatchdog.resrc events are reported when a critical system resource required for Data ONTAP(R) to continue serving data cannot be allocated for an extended period of time, and corrective actions typically involve following the documented corrective action guidance.
