...
[.lead]
clam.mailbox events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) has written to the high availability (HA) mailboxes that it has initialized and is ready to participate in CLAM quorum, and corrective actions typically involve following the documented corrective action guidance.
