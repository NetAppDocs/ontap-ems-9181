...
[.lead]
sms.src events are reported when a SnapMirror(R) Synchronous operation has successfully cleared the persistent fence (read/write I/O has resumed) on the source volume, and corrective actions typically involve following the documented corrective action guidance.
