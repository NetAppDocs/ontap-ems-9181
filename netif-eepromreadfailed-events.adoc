...
[.lead]
netif.eepromreadfailed events are reported when there is a failure in reading the electronically erasable programmable read-only memory (EEPROM) for a network interface, and corrective actions typically involve following the documented corrective action guidance.
