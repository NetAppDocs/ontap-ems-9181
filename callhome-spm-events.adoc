...
[.lead]
callhome.spm events are reported when a subsystem is not fully operational after one or more restarts and a restart threshold has been reached by the Service Process Manager (SPM), and corrective actions typically involve following the documented corrective action guidance.
