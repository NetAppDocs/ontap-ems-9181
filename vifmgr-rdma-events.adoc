...
[.lead]
vifmgr.rdma events are reported when a logical interface (LIF) is moved from a port that does not support the required remote direct memory access (RDMA) protocols of the LIF, and corrective actions typically involve following the documented corrective action guidance.
