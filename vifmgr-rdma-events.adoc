...
[.lead]
Learn about vifmgr.rdma EMS events, which report when a logical interface (LIF) is moved from a port that does not support the required remote direct memory access (RDMA) protocols of the LIF.
