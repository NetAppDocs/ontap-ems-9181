...
[.lead]
Learn about sfu.auto EMS events, which report when ONTAP(R) software cannot automatically upgrade the firmware on a disk shelf because the hidden 'shelf.fw.auto.update' option is set to off.
