...
[.lead]
ic.linkstatuschange events are reported when the link status changes for a high-availability (HA) interconnect port, and corrective actions typically involve verifying the reported configuration and health state.
