...
[.lead]
moveagent.swapvldb events are reported when a 'volume move' operation finishes moving the volume to the destination but fails to update the volume location database (VLDB), and corrective actions typically involve following the documented corrective action guidance.
