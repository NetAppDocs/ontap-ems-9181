...
[.lead]
nblade.nfsv4blockclient events are reported when an NFSv4 client consumes the maximum allowed store pool resources and the store pool system threshold has also been reached, and corrective actions typically involve following the documented corrective action guidance.
