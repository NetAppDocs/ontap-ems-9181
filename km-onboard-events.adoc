...
[.lead]
km.onboard events are reported when a cluster administrator supplies the wrong Onboard Key Manager cluster-wide passphrase more than three times in a row while attempting to enable Common Criteria (CC) mode, and corrective actions typically involve following the documented corrective action guidance.
