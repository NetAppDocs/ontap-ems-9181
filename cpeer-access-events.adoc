...
[.lead]
cpeer.access events are reported when an access request from a peer cluster is rejected, either because the request is made to a restricted Vserver or to a restricted table, and corrective actions typically involve checking the reported configuration and connectivity.
