...
[.lead]
Learn about cpeer.access EMS events, which report when an access request from a peer cluster is rejected, either because the request is made to a restricted Vserver or to a restricted table.
