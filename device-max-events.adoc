...
[.lead]
Learn about device.max EMS events, which report when Data ONTAP(R) cannot install one or more discovered devices since the device type count is at full capacity.
