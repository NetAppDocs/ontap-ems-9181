...
[.lead]
device.max events are reported when data ONTAP(R) cannot install one or more discovered devices since the device type count is at full capacity, and corrective actions typically involve following the documented corrective action guidance.
