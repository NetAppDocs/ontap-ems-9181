...
[.lead]
tapemc.device events are reported when data ONTAP(R) cannot obtain a command block for sending operations to the device, and corrective actions typically involve following the documented corrective action guidance.
