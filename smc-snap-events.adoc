...
[.lead]
smc.snap events are reported when a SnapMirror operation fails to copy the retention period of a snapshot from the source endpoint to the destination endpoint because the ONTAP(R) version of the destination cluster does not support copying the retention period, and corrective actions typically involve checking the reported configuration and connectivity.
