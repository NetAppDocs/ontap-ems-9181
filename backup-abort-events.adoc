...
[.lead]
backup.abort events are reported when backup event like dump has been aborted and currently ROOLR (Read Only On Line Replica) is running on the Filer, and corrective actions typically involve following the documented corrective action guidance.
