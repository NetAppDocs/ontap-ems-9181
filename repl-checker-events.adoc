...
[.lead]
Learn about repl.checker EMS events, which report when a SnapMirror(R) check command detects a missing block on a SnapMirror(R) destination volume and the volume might be in an inconsistent state that should be fixed as soon as possible.
