...
[.lead]
scsitarget.ispfct events are reported when fibre Channel host bus adapter(HBA) returns an event notification to the driver but the driver did not recognize the event type, and corrective actions typically involve contacting NetApp technical support.
