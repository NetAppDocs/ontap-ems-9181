...
[.lead]
sp.firmware events are reported when neither of the firmware images of the node's Service Processor (SP) or Baseboard Management Controller (BMC) on the primary flash chip can be booted, and corrective actions typically involve contacting NetApp technical support.
