...
[.lead]
sis.default events are reported when this message occurs whenever volume move is done from a non All Flash Optimized node to an All Flash Optimized node and we attempt to enable storage efficiency features and automatic offline volume efficiency, and corrective actions typically involve following the documented corrective action guidance.
