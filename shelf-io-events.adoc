---
sidebar: sidebar
permalink: shelf-io-events.html
keywords: ems, message, messages, event, events, group, catalog
summary: shelf.io events
---

= shelf.io events
:toc: macro
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
shelf.io events are reported when a request cannot be performed because I/O is still in progress, and corrective actions typically involve following the documented corrective action guidance.

== shelf.IO.in.progress
*Severity*::
NOTICE
*Description*::
This message occurs when a request cannot be performed because I/O is still in progress.
*Corrective Action*::
(None).
*Syslog Message*::
Could not perform the request; I/O to disk shelf is still in progress.
*Parameters*::
(None).
