...
[.lead]
Learn about scsi.security EMS events, which report when a self-encrypting drive (SED) reports excessive sequential failed authentication attempts with an incorrect Authentication Key (AK).
