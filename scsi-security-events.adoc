...
[.lead]
scsi.security events are reported when a self-encrypting drive (SED) reports excessive sequential failed authentication attempts with an incorrect Authentication Key (AK), and corrective actions typically involve following the documented corrective action guidance.
