...
[.lead]
scsitarget.fct events are reported when a Fibre Channel (FC) Registered State Change Notification (RSCN) frame is received, specifically for a fabric name change event, and corrective actions typically involve following the documented corrective action guidance.
