...
[.lead]
smbc.fanout events are reported when oNTAP(R) detects that source volumes of SnapMirror(R) active sync relationships are involved in an async fanout protection, but the two destination storage virtual machine (SVM)s are not peered with each other, and corrective actions typically involve following the documented corrective action guidance.
