...
[.lead]
Learn about smbc.fanout EMS events, which report when ONTAP(R) detects that source volumes of SnapMirror(R) active sync relationships are involved in an async fanout protection, but the two destination storage virtual machine (SVM)s are not peered with each other.
