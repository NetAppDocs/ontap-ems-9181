...
[.lead]
Learn about diskown.stealingdiskprevented EMS events, which report when a "storage disk assign" operation is performed on a disk that is owned and has a reservation from another system.
