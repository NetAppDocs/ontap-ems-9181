...
[.lead]
sasmon.adapter events are reported when a Serial Attached SCSI (SAS) transceiver (phy) attached to a SAS host bus adapter (HBA) is disabled due to one of the following reasons: exceeded loss of double word synchronization error threshold; exceeded running disparity error threshold; exceeded invalid double word error threshold; exceeded phy reset problem threshold; exceeded broadcast change threshold, and corrective actions typically involve following the documented corrective action guidance.
