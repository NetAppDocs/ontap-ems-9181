...
[.lead]
acp.commerrpersist events are reported when the Alternate Control Path (ACP) Administrator determines that a SAS shelf I/O module (IOM) configured for in-band ACP is experiencing persistent communication errors, and corrective actions typically involve following the documented corrective action guidance.
