...
[.lead]
Learn about acp.commerrpersist EMS events, which report when the Alternate Control Path (ACP) Administrator determines that a SAS shelf I/O module (IOM) configured for in-band ACP is experiencing persistent communication errors.
