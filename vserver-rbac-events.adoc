...
[.lead]
vserver.rbac events are reported when a custom role associated with a Storage Virtual Machine (SVM) has an aliased command directory associated with it, and corrective actions typically involve following the documented corrective action guidance.
