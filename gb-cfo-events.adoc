...
[.lead]
gb.cfo events are reported when a high-availability (HA) controller failover (CFO) giveback is canceled due to RAID activity on a partner aggregate, and corrective actions typically involve following the documented corrective action guidance.
