...
[.lead]
Learn about mav.disabled EMS events, which report when multi-admin verification (MAV) is enabled, the bootarg "bootarg.multi_admin.enable" is set to false, and an operation that normally requires MAV approval is executed via the console.
