...
[.lead]
cloud.bin events are reported when the cloud bin object copy operation (which copies objects from a source object store to a destination object store) fails because it is unable to read source objects due to unauthorized access to the object store container, and corrective actions typically involve ensuring the required configuration and resources are in place.
