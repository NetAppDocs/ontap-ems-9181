...
[.lead]
Learn about nvmens.smbc EMS events, which report when an NVMe namespace is partially restored to a SnapMirror(R) active sync volume because the SnapMirror(R) active sync volume does not support NVMe namespace.
