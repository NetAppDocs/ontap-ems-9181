...
[.lead]
nvmens.smbc events are reported when an NVMe namespace is partially restored to a SnapMirror(R) active sync volume because the SnapMirror(R) active sync volume does not support NVMe namespace, and corrective actions typically involve following the documented corrective action guidance.
