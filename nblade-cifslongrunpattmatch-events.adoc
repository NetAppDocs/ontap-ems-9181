...
[.lead]
nblade.cifslongrunpattmatch events are reported when a CIFS query directory command that includes wildcard characters exceeds the reporting time threshold for completion, and corrective actions typically involve checking the reported configuration and connectivity.
