...
[.lead]
pha.add events are reported when an attempt to add a passthrough host adapter (PHA) disk to a virtual disk adapter fails because the supplied disk number exceeds the maximum supported disk ID, and corrective actions typically involve following the documented corrective action guidance.
