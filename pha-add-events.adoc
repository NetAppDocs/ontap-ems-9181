...
[.lead]
Learn about pha.add EMS events, which report when an attempt to add a passthrough host adapter (PHA) disk to a virtual disk adapter fails because the supplied disk number exceeds the maximum supported disk ID.
