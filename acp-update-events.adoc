...
[.lead]
acp.update events are reported when the Alternate Control Path (ACP) Administrator receives notification from the ACP Processor that it cannot upgrade the firmware because it is in some faulty state or the firmware file is not valid, and corrective actions typically involve following the documented corrective action guidance.
