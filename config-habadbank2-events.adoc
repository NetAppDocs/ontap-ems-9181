...
[.lead]
config.habadbank2 events are reported when a local host adapter is connected to a different switch bank (on ports 0-7) than other adapters on this controller, which are connected to ports 8-15, and corrective actions typically involve following the documented corrective action guidance.
