...
[.lead]
Learn about lmgr.sf EMS events, which report when the lock manager non-disruption subsystem for CIFS continuously available (CA) shares notifies the high-availability (HA) module that transition into the up state is allowed.
