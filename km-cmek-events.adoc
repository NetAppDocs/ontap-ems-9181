...
[.lead]
km.cmek events are reported when the configured Azure Key Vault Key Management Service (AKV-KMS) transitions to the active state after previously being in the blocked state, and corrective actions typically involve following the documented corrective action guidance.
