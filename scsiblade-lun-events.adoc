...
[.lead]
scsiblade.lun events are reported when the system detects a change in identifiers (serial number, UUID, device ID) of a LUN, and corrective actions typically involve following the documented corrective action guidance.
