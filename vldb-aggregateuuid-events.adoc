...
[.lead]
vldb.aggregateuuid events are reported when an aggregate's Universally Unique Identifier (UUID) in the Volume Location Database does not match the aggregate's current UUID, and corrective actions typically involve following the documented corrective action guidance.
