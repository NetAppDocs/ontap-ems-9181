...
[.lead]
nvmeof.onlyoneport events are reported when there is no longer only one port cabled from a node to a shelf, and corrective actions typically involve following the documented corrective action guidance.
