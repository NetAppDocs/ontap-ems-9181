...
[.lead]
Learn about configbr.nodesturl EMS events, which report when the cluster backup is saved on only one node and no offsite configuration backup destination URL is configured.
