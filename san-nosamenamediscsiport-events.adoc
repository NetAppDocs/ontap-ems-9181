...
[.lead]
san.nosamenamediscsiport events are reported when a MetroCluster(TM) configuration cannot be implemented, because the destination cluster for an IP logical interface (LIF) does not appear to contain a port with the same name as the one used within the source cluster, and corrective actions typically involve checking the reported configuration and connectivity.
