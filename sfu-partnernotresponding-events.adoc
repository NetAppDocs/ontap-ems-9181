...
[.lead]
Learn about sfu.partnernotresponding EMS events, which report when one node in a high-availability (HA) configuration does not respond to firmware download requests from another node, in which case the other node cannot download disk shelf firmware.
