...
[.lead]
Learn about zapi.sf EMS events, which report when the M-host layer is operational and has notified the high-availability (HA) subsystem, which can then proceed with certain storage failover operations, such as giveback.
