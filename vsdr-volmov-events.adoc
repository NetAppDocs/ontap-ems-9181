...
[.lead]
vsdr.volmov events are reported when this message occurs during a SnapMirror(R) resync operation on a Storage Virtual Machine (SVM) disaster recovery (DR) relationship, when the MSID of a volume on the source SVM is the same as the MSID of a temporary volume on the destination SVM and the temporary volume is created for a volume move operation, and corrective actions typically involve following the documented corrective action guidance.
