...
[.lead]
Learn about nblade.nfspathresmaxlinks EMS events, which report when This message occurs while resolving an input path, when the server has reached the limit of maximum links it can follow and the corresponding RPC fails.
