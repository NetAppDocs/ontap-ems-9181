...
[.lead]
cf.badshelfcountversion events are reported when this event occurs if we are sent a negotiated failover shelf count message with a version which we don't support, and corrective actions typically involve following the documented corrective action guidance.
