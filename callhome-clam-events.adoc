...
[.lead]
callhome.clam events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveness, Availability Monitor (CLAM) determines that one of the nodes is out of "cluster quorum", and corrective actions typically involve following the documented corrective action guidance.
