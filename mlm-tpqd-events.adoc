...
[.lead]
mlm.tpqd events are reported when data ONTAP(R) has lowered the number of I/O commands that are allowed to be active by at least 25 percent for an array's target port, and corrective actions typically involve following the documented corrective action guidance.
