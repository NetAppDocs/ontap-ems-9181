...
[.lead]
monitor.shutdown events are reported when an automatic shutdown sequence is initiated due to a degraded RAID group that cannot be reconstructed because there are insufficient appropriate spare disks, and corrective actions typically involve following the documented corrective action guidance.
