...
[.lead]
mgwd.cluscheck events are reported when the system changes the cluster LIFs used for internal replicated database (RDB) operations, ensuring they are connected to different cluster switches for optimal cluster resiliency, and corrective actions typically involve following the documented corrective action guidance.
