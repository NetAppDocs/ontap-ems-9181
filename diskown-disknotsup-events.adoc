...
[.lead]
Learn about diskown.disknotsup EMS events, which report when newly inserted disks are only supported with software disk ownership enabled, but it is not enabled.
