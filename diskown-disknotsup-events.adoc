...
[.lead]
diskown.disknotsup events are reported when newly inserted disks are only supported with software disk ownership enabled, but it is not enabled, and corrective actions typically involve following the documented corrective action guidance.
