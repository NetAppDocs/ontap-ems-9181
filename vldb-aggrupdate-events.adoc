...
[.lead]
Learn about vldb.aggrupdate EMS events, which report when This message occurs in a MetroCluster(tm) configuration, when the blade UUID (Universal Unique Identifier) is updated on the local cluster, but a failure prevents the aggregate ownership update from being replicated to the remote cluster.
