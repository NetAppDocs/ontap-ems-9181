...
[.lead]
moveagent.commitdst events are reported when a 'volume move' operation finishes moving the volume to the destination but fails to update the appropriate information on the destination, and corrective actions typically involve following the documented corrective action guidance.
