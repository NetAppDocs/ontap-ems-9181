...
[.lead]
nblade.cifsetcdactionfailed events are reported when the DMAP container encounters an error processing an etcd request, such as a connection failure, and corrective actions typically involve checking the reported configuration and connectivity.
