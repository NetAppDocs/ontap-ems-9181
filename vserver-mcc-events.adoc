...
[.lead]
vserver.mcc events are reported when this message occurs in a MetroCluster(TM) configuration when an attempt to create a Vserver on the partner cluster fails, and corrective actions typically involve following the documented corrective action guidance.
