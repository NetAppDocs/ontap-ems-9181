...
[.lead]
vifmgr.cdb events are reported when attempting to merge a LIF from the configuration database (CDB) to the replicated database (RDB), and the LIF from the CDB is found to have the same address as an existing LIF in the RDB, and corrective actions typically involve following the documented corrective action guidance.
