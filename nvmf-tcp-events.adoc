...
[.lead]
nvmf.tcp events are reported when there is an internal error while creating a NVMe-over-TCP (NVMe/TCP) logical network interface (LIF), and corrective actions typically involve following the documented corrective action guidance.
