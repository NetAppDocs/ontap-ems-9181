...
[.lead]
nblade.vscanconninvaliduser events are reported when this message occurs during a client attempt to establish a vscan connection, when the logged-in user of that client is not found in the list of allowed users, and corrective actions typically involve ensuring the required configuration and resources are in place.
