...
[.lead]
sp.heartbeat events are reported when the appliance detects resumption of Service Processor (SP) heartbeat notifications indicating that the Service Processor is now available, and corrective actions typically involve following the documented corrective action guidance.
