...
[.lead]
cf.nopartner_takeover events are reported when the partner node fails but requested not to be taken over to prevent multiple, simultaneous RAID reconstructs from starting, and corrective actions typically involve checking the reported configuration and connectivity.
