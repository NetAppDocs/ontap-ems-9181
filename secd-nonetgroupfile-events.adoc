...
[.lead]
Learn about secd.nonetgroupfile EMS events, which report when trying to process a netgroup in an export policy while the Vserver "ns-switch" option is set exclusively to "file" but no netgroup file is loaded.
