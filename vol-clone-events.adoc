...
[.lead]
Learn about vol.clone EMS events, which report when the clone split of a volume clone takes the optimized path which uses an Azure copy blob operation to copy objects from the parent volume to the clone.
