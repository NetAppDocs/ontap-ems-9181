...
[.lead]
vol.clone events are reported when the clone split of a volume clone takes the optimized path which uses an Azure copy blob operation to copy objects from the parent volume to the clone, and corrective actions typically involve following the documented corrective action guidance.
