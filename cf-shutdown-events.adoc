...
[.lead]
cf.shutdown events are reported when the controller receives a message from its partner over the HA interconnect that indicates a condition related to the partner's attempted graceful shutdown, and corrective actions typically involve following the documented corrective action guidance.
