...
[.lead]
fmmb.oldformatcleared events are reported when this event is issued when the older format HA mailboxes (special area of disk used for bookkeeping) are cleared on an upgrade from a pre 7, and corrective actions typically involve following the documented corrective action guidance.
