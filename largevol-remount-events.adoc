...
[.lead]
Learn about largevol.remount EMS events, which report when the "is-large-size-enabled" property is set on a FlexVol volume or Flexgroup volume, which is NFS mounted for client access.
