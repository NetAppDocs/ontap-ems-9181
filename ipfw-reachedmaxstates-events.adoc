...
[.lead]
ipfw.reachedmaxstates events are reported when the ipfw firewall fails to create a new dynamic state entry for a "keep-state" rule because the number of dynamic state entries has reached the maximum allowed value of 32768, and corrective actions typically involve following the documented corrective action guidance.
