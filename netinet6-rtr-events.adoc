...
[.lead]
netinet6.rtr events are reported when the Maximum Transmission Unit (MTU) size of the Router Advertisement (RA) message, sent from the specified source IPv6 address, is higher than the upper bound of the Link MTU, and corrective actions typically involve following the documented corrective action guidance.
