...
[.lead]
Learn about netif.badeeprom EMS events, which report when the network interface has an electronically erasable programmable read-only memory (EEPROM) with a bad cyclic redundancy check (CRC) or media access control (MAC) address.
