...
[.lead]
netif.badeeprom events are reported when the network interface has an electronically erasable programmable read-only memory (EEPROM) with a bad cyclic redundancy check (CRC) or media access control (MAC) address, and corrective actions typically involve following the documented corrective action guidance.
