...
[.lead]
net.ifconfig events are reported when the limit for adding logical virtual local area network (VLAN) and interface group (ifgrp) interfaces is exceeded, and corrective actions typically involve following the documented corrective action guidance.
