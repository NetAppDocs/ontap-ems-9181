...
[.lead]
Learn about san.noconnectediscsiport EMS events, which report when This message occurs in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP cannot replicate an IP logical interface (LIF) to a candidate port in the destination cluster.
