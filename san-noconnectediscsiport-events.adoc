...
[.lead]
san.noconnectediscsiport events are reported when this message occurs in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP cannot replicate an IP logical interface (LIF) to a candidate port in the destination cluster, and corrective actions typically involve checking the reported configuration and connectivity.
