...
[.lead]
mgmtgwd.rootvol events are reported when the system detects that a root volume might have changed, causing the local management databases to be out of sync with the replicated databases, and corrective actions typically involve contacting NetApp technical support.
