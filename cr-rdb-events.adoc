...
[.lead]
cr.rdb events are reported when the RDB (replicated database) cannot be updated with the inode counters for a particular scan, and corrective actions typically involve following the documented corrective action guidance.
