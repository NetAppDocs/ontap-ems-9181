...
[.lead]
callhome.multi events are reported when checksum verification errors are seen on two or more disks connected to the same HBA port, and the port is not taken offline or the "disk, and corrective actions typically involve contacting NetApp technical support.
