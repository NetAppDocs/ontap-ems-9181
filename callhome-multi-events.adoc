...
[.lead]
Learn about callhome.multi EMS events, which report when checksum verification errors are seen on two or more disks connected to the same HBA port, and the port is not taken offline or the "disk.offline_adapter.enable" option is disabled.
