...
[.lead]
mcc.switchover events are reported when a MetroCluster switchover operation finds a system metadata volume, used by config replication applications, in the nvfailed state, and corrective actions typically involve verifying the reported configuration and health state.
