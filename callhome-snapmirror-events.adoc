...
[.lead]
callhome.snapmirror events are reported when a sync-granular consistency group (CG) transitions from in-sync status to out-of-sync status, and corrective actions typically involve following the documented corrective action guidance.
