...
[.lead]
nblade.tlsconfigerror events are reported when the client is trying to use NFS over TLS (Transport Layer Security), but it is disabled on the NFS server side, and corrective actions typically involve following the documented corrective action guidance.
