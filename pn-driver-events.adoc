...
[.lead]
pn.driver events are reported when the pass-through network layer detects driver send latencies that are too high, indicating a noisy neighbor issue in cloud providers that is causing an I/O latency spike, and corrective actions typically involve following the documented corrective action guidance.
