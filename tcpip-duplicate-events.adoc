...
[.lead]
tcpip.duplicate events are reported when the local node detects that it and some other device are using the same IP address, and corrective actions typically involve checking the reported configuration and connectivity.
