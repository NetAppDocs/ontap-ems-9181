...
[.lead]
Learn about vifmgr.unabletohostmovedlif EMS events, which report when an attempt to move a logical interface (LIF) IP address to a specific node/port failed, and a subsequent attempt was made to move that LIF to another location.
