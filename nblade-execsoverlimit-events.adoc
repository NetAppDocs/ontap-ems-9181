...
[.lead]
nblade.execsoverlimit events are reported when the number of active requests to a connection exceeds the allowed in-flight request threshold, which results in request throttling, and corrective actions typically involve checking the reported configuration and connectivity.
