...
[.lead]
moveagent.switchback events are reported when the volume move operation vetoes a MetroCluster(tm) switchback because the operations are mutually exclusive, and corrective actions typically involve following the documented corrective action guidance.
