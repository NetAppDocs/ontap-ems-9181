...
[.lead]
callhome.shlf events are reported when the system detects faulty hardware on the disk shelf, such as a fan, power supply unit (PSU), or failing temperature sensor, and corrective actions typically involve following the documented corrective action guidance.
