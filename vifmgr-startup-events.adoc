...
[.lead]
vifmgr.startup events are reported when the logical interface manager (VIFMgr) encounters an error while initializing the structures required for logical interface (LIF) failover, and corrective actions typically involve contacting NetApp technical support.
