...
[.lead]
disk.encryptfipspassphnosync events are reported when a system with NetApp(R) Storage Encryption (NSE) enabled detects that the authentication keys (AKs, or passphrases) for the internal FIPS-compliance authorities in a FIPS-certified drive are inconsistent, and corrective actions typically involve following the documented corrective action guidance.
