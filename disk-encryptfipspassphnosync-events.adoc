...
[.lead]
Learn about disk.encryptfipspassphnosync EMS events, which report when a system with NetApp(R) Storage Encryption (NSE) enabled detects that the authentication keys (AKs, or passphrases) for the internal FIPS-compliance authorities in a FIPS-certified drive are inconsistent.
