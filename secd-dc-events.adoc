...
[.lead]
secd.dc events are reported when the Windows Domain Controller (DC), while responding to an MSRPC request from Data ONTAP(R), returns an error indicating that it ran out of pipe instances in the listening state, and corrective actions typically involve contacting NetApp technical support.
