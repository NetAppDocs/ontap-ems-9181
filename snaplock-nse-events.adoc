...
[.lead]
snaplock.nse events are reported when any of the following commands could not be run against a SnapLock Compliance disk, since doing so could risk the destruction of compliance data: "storage encryption disk sanitize", "storage encryption disk destroy", "storage encryption disk revert-to-original-state", and corrective actions typically involve following the documented corrective action guidance.
