...
[.lead]
Learn about snaplock.nse EMS events, which report when any of the following commands cannot be run against a SnapLock Compliance disk, since doing so could risk the destruction of compliance data: "storage encryption disk sanitize", "storage encryption disk destroy", "storage encryption disk revert-to-original-state".
