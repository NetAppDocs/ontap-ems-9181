...
[.lead]
Learn about wafl.onoff EMS events, which report when WAFL(R) attempts to bring online or take offline a mirror volume that does not have a valid mirror-type attribute.This situation might be a direct result of an upgrade to a version of Data ONTAP(R) that requires the mirror-type attribute or as a result of a corruption.
