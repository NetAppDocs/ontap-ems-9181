...
[.lead]
ha.netpartition events are reported when a Data ONTAP(R) subsystem detects an error potentially due to network partitioning, but cannot take over the partner node or allow the partner to take over because both takeover-of-partner and takeover-by-partner are both disabled, and corrective actions typically involve following the documented corrective action guidance.
