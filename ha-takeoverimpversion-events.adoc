...
[.lead]
ha.takeoverimpversion events are reported when the partner node is down; however, due to a version mismatch, it is currently impossible to perform a takeover, and corrective actions typically involve following the documented corrective action guidance.
