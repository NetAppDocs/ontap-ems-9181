...
[.lead]
qos.min events are reported when minimum throughput value is not honoured if adaptive QoS is set on a SnapMirror Synchronous protected storage object, and corrective actions typically involve following the documented corrective action guidance.
