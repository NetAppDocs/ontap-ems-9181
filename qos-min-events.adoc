...
[.lead]
Learn about qos.min EMS events, which report when minimum throughput value is not honoured if adaptive QoS is set on a SnapMirror Synchronous protected storage object.
