...
[.lead]
rdb.env events are reported when the cluster management Data Replication Module determines that I/O operations on the node's mroot filesystem are failing, and corrective actions typically involve contacting NetApp technical support.
