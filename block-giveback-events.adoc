...
[.lead]
Learn about block.giveback EMS events, which report when This message occurs during a storage failover (SFO) aggregate giveback, when a SnapMirror(R) active sync automatic unplanned failover operation is in progress.
