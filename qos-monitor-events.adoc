...
[.lead]
qos.monitor events are reported when the Quality of Service (QoS) subsystem's dynamic memory is no longer at its limit for the current platform hardware, and corrective actions typically involve following the documented corrective action guidance.
