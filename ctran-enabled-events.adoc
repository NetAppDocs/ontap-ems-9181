---
sidebar: sidebar
permalink: ctran-enabled-events.html
keywords: ems, message, messages, event, events, group, catalog
summary: ctran.enabled events
---

= ctran.enabled events
:toc: macro
:toclevels: 1
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
ctran.enabled events are reported when kernel Cluster Services (KCS) Cluster Transaction Manager (CTRAN) subsystem is enabled, and corrective actions typically involve following the documented corrective action guidance.

== ctran.enabled
*Severity*::
INFORMATIONAL
*Description*::
This message occurs when Kernel Cluster Services (KCS) Cluster Transaction Manager (CTRAN) subsystem is enabled.
*Corrective Action*::
No action is needed.
*Syslog Message*::
CTRAN subsystem is now enabled.
*Parameters*::
(None).
