...
[.lead]
acp.sock events are reported when the system encounters a socket error while setting up the Alternate Control Path (ACP) Management Ethernet port, and corrective actions typically involve verifying the reported configuration and health state.
