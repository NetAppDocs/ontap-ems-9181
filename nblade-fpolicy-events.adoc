...
[.lead]
nblade.fpolicy events are reported when an I/O operation directed at a FlexCache(R) volume fails because the file extension include and exclude list of one of the policies is too long to send to the origin of the FlexCache volume, and corrective actions typically involve following the documented corrective action guidance.
