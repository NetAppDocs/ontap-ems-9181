...
[.lead]
ctran.net events are reported when the Cluster Transaction Manager (CTRAN) on the local node is unable to reach a peer node that it believes is in CLAM quorum, and corrective actions typically involve checking the reported configuration and connectivity.
