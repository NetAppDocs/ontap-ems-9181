...
[.lead]
vsdr.ruleaddedtosmpolicy events are reported when a MAX Data(R) schedule update adds a new rule to a SnapMirror(R) policy with the "mirror-vault" type, and corrective actions typically involve following the documented corrective action guidance.
