...
[.lead]
Learn about fcvi.unsupported EMS events, which report when a network adapter port behavior type is set to use FC-VI on a platform that has not been validated for its use and is therefore unsupported.
