...
[.lead]
secd.authsys events are reported when the incoming UNIX user ID (UID) that tries to mount or access a mount point cannot be looked up in any of the name-services (NIS, LDAP, file), and corrective actions typically involve ensuring the required configuration and resources are in place.
