...
[.lead]
arl.aggronlinefailed events are reported when the RAID or WAFL(R) subsystem fails to bring online the aggregate or its containing volumes after relocation, and corrective actions typically involve checking the reported configuration and connectivity.
