...
[.lead]
ssh.sshrsa events are reported when this message informs the user that future ONTAP releases will, by default, disable the "ssh-rsa (RSA/SHA-1)" form of the signature scheme used with RSA keys for host-based authentication and publickey-based user authentication, and corrective actions typically involve following the documented corrective action guidance.
