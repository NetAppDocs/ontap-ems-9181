...
[.lead]
config.nobconnect events are reported when the local node in an HA configuration has no access to its partner node's disk shelves because the partner node's shelves are not connected to the local node through the shelf's module B, and corrective actions typically involve following the documented corrective action guidance.
