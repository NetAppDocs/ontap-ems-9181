...
[.lead]
Learn about config.nobconnect EMS events, which report when the local node in an HA configuration has no access to its partner node's disk shelves because the partner node's shelves are not connected to the local node through the shelf's module B.
