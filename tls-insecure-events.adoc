...
[.lead]
tls.insecure events are reported when during the TLS handshake a peer negotiates a cipher suite that is no longer considered sufficiently secure to be enabled in the default configuration, and corrective actions typically involve following the documented corrective action guidance.
