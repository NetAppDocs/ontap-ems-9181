...
[.lead]
vifmgr.lifidoutofsync events are reported when a Logical Interface (LIF) is configured in the Logical Interface manager (VIFMgr) but is not visible in the user interface, and corrective actions typically involve following the documented corrective action guidance.
