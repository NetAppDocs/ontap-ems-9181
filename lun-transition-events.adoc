...
[.lead]
lun.transition events are reported when the transition of a 7-Mode LUN to clustered Data ONTAP(R) fails because the i2p database for the destination volume is not fully built, and corrective actions typically involve following the documented corrective action guidance.
