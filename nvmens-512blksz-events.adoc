...
[.lead]
Learn about nvmens.512blksz EMS events, which report when replication of a volume containing NVMe namespaces that use 512-byte block sizes fails because the destination does not support NVMe namespaces with this block size.
