...
[.lead]
nvmens.512blksz events are reported when replication of a volume containing NVMe namespaces that use 512-byte block sizes fails because the destination does not support NVMe namespaces with this block size, and corrective actions typically involve following the documented corrective action guidance.
