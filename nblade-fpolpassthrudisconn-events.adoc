...
[.lead]
nblade.fpolpassthrudisconn events are reported when the pass-through read channel between the node and the FPolicy server is disconnected, and corrective actions typically involve following the documented corrective action guidance.
