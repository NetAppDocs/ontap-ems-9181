...
[.lead]
scsiblade.dpa events are reported when the node loses connectivity with the majority of the cluster and the Data Plane Availability feature has been explicitly disabled by the user, and corrective actions typically involve following the documented corrective action guidance.
