...
[.lead]
netinet6.icmp events are reported when the system receives an Internet Control Message Protocol(ICMP) message, and while processing the ICMP message it finds a checksum error, and corrective actions typically involve following the documented corrective action guidance.
