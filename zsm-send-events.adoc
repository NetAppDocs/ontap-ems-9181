...
[.lead]
zsm.send events are reported when a socket timeout occurs on an intracluster Manage ONTAP(R) API request to an internal ONTAPI(R) server, and corrective actions typically involve checking the reported configuration and connectivity.
