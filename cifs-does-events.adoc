...
[.lead]
cifs.does events are reported when a Vserver or Cluster administrator tries to initialize Snapmirror on Async VserverDR ID Discard setup without creating a CIFS server at destination, and corrective actions typically involve following the documented corrective action guidance.
