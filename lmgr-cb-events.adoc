...
[.lead]
Learn about lmgr.cb EMS events, which report when a lock manager waiting lock callback object cannot be allocated due to reaching the limit for waiting lock callback objects in the associated pool.
