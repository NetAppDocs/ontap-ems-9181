...
[.lead]
lmgr.cb events are reported when a lock manager waiting lock callback object cannot be allocated due to reaching the limit for waiting lock callback objects in the associated pool, and corrective actions typically involve following the documented corrective action guidance.
