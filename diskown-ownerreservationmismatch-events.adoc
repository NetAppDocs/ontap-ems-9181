...
[.lead]
Learn about diskown.ownerreservationmismatch EMS events, which report when the system discovers a disk that is supposed to be owned by one node, but the persistent reservation is owned by another node.
