...
[.lead]
diskown.ownerreservationmismatch events are reported when the system discovers a disk that is supposed to be owned by one node, but the persistent reservation is owned by another node, and corrective actions typically involve following the documented corrective action guidance.
