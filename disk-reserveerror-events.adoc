...
[.lead]
disk.reserveerror events are reported when a SCSI reservation is requested for a disk or LUN that is no longer seen, and corrective actions typically involve following the documented corrective action guidance.
