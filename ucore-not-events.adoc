...
[.lead]
Learn about ucore.not EMS events, which report when a userspace core cannot be dumped either because coredumps are not enabled or because the process called setuid, and coredump is disabled for processes that have changed their user credentials.
