...
[.lead]
ucore.not events are reported when a userspace core cannot be dumped either because coredumps are not enabled or because the process called setuid, and coredump is disabled for processes that have changed their user credentials, and corrective actions typically involve contacting NetApp technical support.
