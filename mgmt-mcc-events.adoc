...
[.lead]
mgmt.mcc events are reported when metroCluster(tm) switchback or switchover happens while applying a file security (fsecurity) policy on the MetroCluster partner, and corrective actions typically involve following the documented corrective action guidance.
