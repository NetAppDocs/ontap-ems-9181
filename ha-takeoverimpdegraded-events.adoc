...
[.lead]
ha.takeoverimpdegraded events are reported when the partner node is down and it is currently impossible to perform a takeover because the partner mailbox disks are not available, and corrective actions typically involve checking the reported configuration and connectivity.
