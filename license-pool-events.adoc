...
[.lead]
license.pool events are reported when a storage failover giveback is vetoed because the aggregate has no valid capacity lease from the capacity pool, and corrective actions typically involve following the documented corrective action guidance.
