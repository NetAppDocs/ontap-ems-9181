...
[.lead]
flexcache.originfull_1 events are reported when a FlexCache(R) volume cannot retrieve data from its origin volume because origin volume is full, and corrective actions typically involve ensuring the required configuration and resources are in place.
