...
[.lead]
callhome.link events are reported when a network port link state change cannot be confirmed for more than 5 minutes because another configuration task is still in progress, and corrective actions typically involve following the documented corrective action guidance.
