...
[.lead]
Learn about mgmt.vdr EMS events, which report when a "snapmirror break" operation is executed while a file security job is running and the previous "snapmirror update" operation only partially transfers the changes made to the file system by the file security job.
