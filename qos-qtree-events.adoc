...
[.lead]
Learn about qos.qtree EMS events, which report when the Quality of Service (QoS) subsystem recognizes that an older version of a filehandle is still being used to access a qtree associated with a QoS policy group.
