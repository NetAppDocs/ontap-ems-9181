...
[.lead]
Learn about rbac.spuser EMS events, which report when the role that was earlier associated with the service-processor application user in a previous release was deleted in the current release and is not found during a revert to a previous release.
