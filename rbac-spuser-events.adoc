...
[.lead]
rbac.spuser events are reported when the role that was earlier associated with the service-processor application user in a previous release was deleted in the current release and is not found during a revert to a previous release, and corrective actions typically involve following the documented corrective action guidance.
