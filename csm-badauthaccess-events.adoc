...
[.lead]
csm.badauthaccess events are reported when a blade fails an attempt to authenticate as a given peer cluster, and Cluster Session Manager (CSM), which establishes and manages SpinNP sessions between blades in the cluster, denies access of the unauthenticated blade, and corrective actions typically involve following the documented corrective action guidance.
