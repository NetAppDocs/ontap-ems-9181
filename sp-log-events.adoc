...
[.lead]
sp.log events are reported when the available memory of the log partition in the Service Processor (SP) or the Baseboard Management Controller (BMC) is low, and corrective actions typically involve following the documented corrective action guidance.
