...
[.lead]
disk.encryptcmdfailed events are reported when the indicated "storage encryption disk" command fails on an encrypting disk that supports storage encryption methods, and corrective actions typically involve verifying the reported configuration and health state.
