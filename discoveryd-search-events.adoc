...
[.lead]
Learn about discoveryd.search EMS events, which report when the discoveryd service is contacted during the initial cluster setup phase with an M-SEARCH multicast and a follow up HTTP GET to get the device description document.
