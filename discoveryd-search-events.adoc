...
[.lead]
discoveryd.search events are reported when the discoveryd service is contacted during the initial cluster setup phase with an M-SEARCH multicast and a follow up HTTP GET to get the device description document, and corrective actions typically involve following the documented corrective action guidance.
