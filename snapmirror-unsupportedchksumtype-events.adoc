...
[.lead]
snapmirror.unsupportedchksumtype events are reported when checksum type crc32c_header_only is configured for a SnapMirror(R) relationship and the source for the relationship is not compatible with this checksum type, and corrective actions typically involve following the documented corrective action guidance.
