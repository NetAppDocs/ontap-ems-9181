...
[.lead]
wafl.relocating events are reported when the system cannot perform an aggregate relocation because a fence is set on one of the volumes in the aggregate, and corrective actions typically involve following the documented corrective action guidance.
