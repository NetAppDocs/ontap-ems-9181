...
[.lead]
Learn about sfo.reassignnopartner EMS events, which report when This message occurs during the Storage Failover takeover process, when the disks from the failed node cannot be reassigned because they are unaccessible to the surviving node.
