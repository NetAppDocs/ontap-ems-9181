...
[.lead]
Learn about san.mccvserverinitfailed EMS events, which report when This message occurs in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems when SAN Vserver failed to initialize during a switchover or switchback operation.
