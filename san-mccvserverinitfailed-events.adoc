...
[.lead]
san.mccvserverinitfailed events are reported when this message occurs in MetroCluster(TM) configurations on clustered Data ONTAP(R) systems when SAN Vserver failed to initialize during a switchover or switchback operation, and corrective actions typically involve contacting NetApp technical support.
