...
[.lead]
vldb.adtcons events are reported when the Remote Procedure Call (RPC) from the Volume Location Database (VLDB) to the Management Gateway (MGWD) fails, and corrective actions typically involve following the documented corrective action guidance.
