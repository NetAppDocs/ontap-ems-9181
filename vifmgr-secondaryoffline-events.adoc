...
[.lead]
Learn about vifmgr.secondaryoffline EMS events, which report when the system determines that a remote node has been offline for more than a minute and assigns logical network interfaces (LIFs) from that node to other available nodes in the cluster.
