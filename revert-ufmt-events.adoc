...
[.lead]
revert.ufmt events are reported when the revert operation to convert the unified object format is paused because the FabricPool aggregate has reached the maximum supported size of the capacity tier, and corrective actions typically involve contacting NetApp technical support.
