...
[.lead]
replication.qtree events are reported when the replication destination engine receives blocks with disk errors from the source for the first time in a transfer and continues with replication, and corrective actions typically involve contacting NetApp technical support.
