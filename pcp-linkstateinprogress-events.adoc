...
[.lead]
pcp.linkstateinprogress events are reported when a change of network port state cannot be confirmed because another configuration task is in progress, and corrective actions typically involve following the documented corrective action guidance.
