...
[.lead]
fpolicy.memory events are reported when either the FPolicy Service Manager (FSM) receives too many requests to process immediately, or when the number of requests is accumulating due to server connectivity issues, and corrective actions typically involve checking the reported configuration and connectivity.
