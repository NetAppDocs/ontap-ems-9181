...
[.lead]
vsdr.volmovsrcfpnonfpaggr events are reported when a volume move operation is executed between FabricPool and non-FabricPool aggregates on the source Vserver of an SVM disaster recovery (SVMDR) relationship, and corrective actions typically involve following the documented corrective action guidance.
