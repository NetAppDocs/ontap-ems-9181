...
[.lead]
Learn about vsdr.volmovsrcfpnonfpaggr EMS events, which report when a volume move operation is executed between FabricPool and non-FabricPool aggregates on the source Vserver of an SVM disaster recovery (SVMDR) relationship.
