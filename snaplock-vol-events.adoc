...
[.lead]
snaplock.vol events are reported when the SnapLock(R) volume compliance clock time is skewed from the SnapLock system compliance clock by more than one day, and corrective actions typically involve following the documented corrective action guidance.
