...
[.lead]
Learn about san.fcnvmeunsupportedport EMS events, which report when This message occurs in MetroCluster(tm) configurations on clustered Data ONTAP(R) systems, when Data ONTAP has an FC NVMe (Nonvolatile Memory Express) logical interface (LIF) hosted on an unsupported FC port.
