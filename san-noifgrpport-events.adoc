...
[.lead]
Learn about san.noifgrpport EMS events, which report when This message occurs in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP places a LIF on a physical port because an ifgrp port cannot be found on the destination cluster node (DR partner) to host an IP LIF.
