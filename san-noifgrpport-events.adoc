...
[.lead]
san.noifgrpport events are reported when this message occurs in MetroCluster(TM) configurations on clustered ONTAP(R) systems, when ONTAP places a LIF on a physical port because an ifgrp port cannot be found on the destination cluster node (DR partner) to host an IP LIF, and corrective actions typically involve ensuring the required configuration and resources are in place.
