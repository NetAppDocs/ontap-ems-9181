...
[.lead]
osc.bucket events are reported when the object-store bucket or container is not found while trying to access an object, and corrective actions typically involve verifying the reported configuration and health state.
