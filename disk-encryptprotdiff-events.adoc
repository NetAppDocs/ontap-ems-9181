...
[.lead]
Learn about disk.encryptprotdiff EMS events, which report when self-encrypting drives have different settings of data-at-rest protections; that is, some are protected, having a nondefault Data Key ID, and some are not, having one of the default Data Key IDs.
