...
[.lead]
disk.encryptprotdiff events are reported when self-encrypting drives have different settings of data-at-rest protections; that is, some are protected, having a nondefault Data Key ID, and some are not, having one of the default Data Key IDs, and corrective actions typically involve following the documented corrective action guidance.
