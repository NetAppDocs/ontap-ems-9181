...
[.lead]
disk.encryptgetmsidfailed events are reported when an encrypting disk reports a SCSI data protection error, but Data ONTAP(R) cannot obtain the Manufacturer's Secure ID (MSID) that is mandatory on disks that support storage encryption methods, and corrective actions typically involve following the documented corrective action guidance.
