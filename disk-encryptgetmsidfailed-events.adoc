...
[.lead]
Learn about disk.encryptgetmsidfailed EMS events, which report when an encrypting disk reports a SCSI data protection error, but Data ONTAP(R) cannot obtain the Manufacturer's Secure ID (MSID) that is mandatory on disks that support storage encryption methods.
