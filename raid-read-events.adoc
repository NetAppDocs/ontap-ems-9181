...
[.lead]
raid.read events are reported when the system detects a checksum block media error in an advanced zoned checksums(AZCS) RAID group during a read operation, and corrective actions typically involve following the documented corrective action guidance.
