...
[.lead]
adt.service events are reported when the volume that hosts the destination path where the final audit logs are stored is not accessible or does not have sufficient space and the audit consolidation service is not able to write to the volume, and corrective actions typically involve following the documented corrective action guidance.
