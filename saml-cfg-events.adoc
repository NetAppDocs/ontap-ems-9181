...
[.lead]
saml.cfg events are reported when the Security Assertion Markup Language (SAML) Service Provider (SP) configuration fails to download the Identity Provider (IdP) metadata, and corrective actions typically involve verifying the reported configuration and health state.
