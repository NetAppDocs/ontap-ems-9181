...
[.lead]
dfu.suspended_disasterrecovery events are reported when this message occurs in a MetroCluster(TM) environment when firmware download to remote disks is suspended during disaster recovery, and corrective actions typically involve following the documented corrective action guidance.
