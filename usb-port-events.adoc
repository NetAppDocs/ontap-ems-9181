...
[.lead]
usb.port events are reported when the Data ONTAP(R) universal serial bus (USB) adapter driver detects an unrecoverable error on the associated port, and corrective actions typically involve following the documented corrective action guidance.
