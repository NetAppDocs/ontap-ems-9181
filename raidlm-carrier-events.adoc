...
[.lead]
raidlm.carrier events are reported when data ONTAP(R) determines that an ongoing disk copy operation is no longer required and has stopped the disk copy previously started for this disk, and corrective actions typically involve following the documented corrective action guidance.
