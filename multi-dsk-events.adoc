...
[.lead]
Learn about multi.dsk EMS events, which report when multiple errors are detected during checksum verification, and the adapter is not taken offline or the "disk.offline_adapter.enable" option is disabled.
