...
[.lead]
snapmirror.reconstruct events are reported when the SnapMirror(R) requested reconstruction of a per-volume meta file used for labeling block information is complete, and corrective actions typically involve following the documented corrective action guidance.
