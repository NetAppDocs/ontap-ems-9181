...
[.lead]
wafl.fsa events are reported when enabling file system analytics on a volume with autosize enabled fails because there is insufficient space in the volume, and corrective actions typically involve following the documented corrective action guidance.
