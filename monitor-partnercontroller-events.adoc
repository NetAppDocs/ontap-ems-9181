...
[.lead]
Learn about monitor.partnercontroller EMS events, which report when both controllers are configured as an HA (high-availability) pair, but the partner controller is not present.
