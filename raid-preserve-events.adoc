...
[.lead]
raid.preserve events are reported when during RAID replay of NVRAM, entries associated with a non-existent (offline or missing) aggregate were discovered, and corrective actions typically involve following the documented corrective action guidance.
