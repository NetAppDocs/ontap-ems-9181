...
[.lead]
Learn about clam.valid EMS events, which report when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) determines that the local node is in a valid configuration for providing CLAM functionality.
