...
[.lead]
config.sas2domain events are reported when a disk is multipathed to one SAS domain; that is, both ports of the disk are visible from the same HBA port, and corrective actions typically involve following the documented corrective action guidance.
