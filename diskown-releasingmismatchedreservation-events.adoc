...
[.lead]
diskown.releasingmismatchedreservation events are reported when this event is issued when we release the reservation because the DOWN says the disk is owned by someone else, and corrective actions typically involve following the documented corrective action guidance.
