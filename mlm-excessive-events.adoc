...
[.lead]
Learn about mlm.excessive EMS events, which report when ONTAP(R) detects that the average SCSI I/O command latency on the target port of the array exceeds the set threshold.
