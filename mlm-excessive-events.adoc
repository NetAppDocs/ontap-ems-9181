...
[.lead]
mlm.excessive events are reported when oNTAP(R) detects that the average SCSI I/O command latency on the target port of the array exceeds the set threshold, and corrective actions typically involve following the documented corrective action guidance.
