...
[.lead]
ses.download events are reported when the power cycling channel event is issued after a disk shelf firmware download to disk shelves that require a power-cycle to activate the new code, and corrective actions typically involve following the documented corrective action guidance.
