...
[.lead]
vifmgr.clusterbd events are reported when oNTAP detects that the cluster broadcast domain is partitioned; some local ports in the cluster broadcast domain do not have layer-2 connectivity to one another, and corrective actions typically involve checking the reported configuration and connectivity.
