...
[.lead]
Learn about vifmgr.clusterbd EMS events, which report when ONTAP detects that the cluster broadcast domain is partitioned; some local ports in the cluster broadcast domain do not have layer-2 connectivity to one another.
