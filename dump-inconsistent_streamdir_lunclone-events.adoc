...
[.lead]
dump.inconsistent_streamdir_lunclone events are reported when the dump engine encounters a disk error in a stream directory of an inconsistent LUN clone in a Snapshot(tm) copy, and corrective actions typically involve contacting NetApp technical support.
