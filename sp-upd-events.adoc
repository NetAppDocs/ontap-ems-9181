...
[.lead]
sp.upd events are reported when the Service Processor (SP) daemon cannot extract the SP firmware version from the metadata file, and corrective actions typically involve following the documented corrective action guidance.
