...
[.lead]
lmgr.partner events are reported when a partner node's mirrored lock state goes out of sync with the actual lock state, and corrective actions typically involve following the documented corrective action guidance.
