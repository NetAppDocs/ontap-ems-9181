...
[.lead]
netgroup.files events are reported when a netgroup lookup request finds that files is specified as a ns-switch source, but a netgroup file cannot be found, and corrective actions typically involve checking the reported configuration and connectivity.
