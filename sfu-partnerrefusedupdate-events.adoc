...
[.lead]
Learn about sfu.partnerrefusedupdate EMS events, which report when This message occurs in a high-availability (HA) configuration in which one node refuses firmware download requests from its partner node, in which case the partner node cannot download disk shelf firmware.
