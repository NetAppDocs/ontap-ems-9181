...
[.lead]
Learn about vldb.aggregatename EMS events, which report when an aggregate's name in the Volume Location Database (VLDB) does not match the aggregate's current name on the node.
