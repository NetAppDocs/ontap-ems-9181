...
[.lead]
vldb.aggregatename events are reported when an aggregate's name in the Volume Location Database (VLDB) does not match the aggregate's current name on the node, and corrective actions typically involve following the documented corrective action guidance.
