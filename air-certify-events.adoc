...
[.lead]
Learn about air.certify EMS events, which report when a metadata inconsistency is discovered inside a file system feature that is read-only and therefore cannot be immediately corrected.
