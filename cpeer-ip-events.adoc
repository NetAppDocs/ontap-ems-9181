...
[.lead]
cpeer.ip events are reported when an attempt is made to insert an IP address whose address family does not match the family style specified for the cluster peer relationship, and corrective actions typically involve following the documented corrective action guidance.
