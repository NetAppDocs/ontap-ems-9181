...
[.lead]
Learn about splog.warnings EMS events, which report when the ONTAP(R) splogd process has been unable to retrieve a log from the Service Processor (SP) or baseboard management controller (BMC) for at least 24 hours.
