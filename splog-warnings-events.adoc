...
[.lead]
splog.warnings events are reported when the ONTAP(R) splogd process has been unable to retrieve a log from the Service Processor (SP) or baseboard management controller (BMC) for at least 24 hours, and corrective actions typically involve following the documented corrective action guidance.
