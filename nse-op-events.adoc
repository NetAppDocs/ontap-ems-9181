...
[.lead]
Learn about nse.op EMS events, which report when ONTAP(R) does not succeed in performing an operation on a NetApp(R) Storage Encryption (NSE) self-encrypting drive (SED).
