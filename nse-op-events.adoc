...
[.lead]
nse.op events are reported when oNTAP(R) does not succeed in performing an operation on a NetApp(R) Storage Encryption (NSE) self-encrypting drive (SED), and corrective actions typically involve following the documented corrective action guidance.
