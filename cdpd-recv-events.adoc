...
[.lead]
cdpd.recv events are reported when the CDP (Cisco Discovery Protocol) daemon running on the storage controller receives more unique neighbor advertisements than it can cache, and corrective actions typically involve following the documented corrective action guidance.
