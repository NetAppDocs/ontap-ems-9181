...
[.lead]
tapemc.passthru events are reported when a data underrun (less data is transferred than requested) happens on a read operation to the device, and the number of data bytes reported as not transferred is unexpectedly larger than the number of bytes requested, and corrective actions typically involve following the documented corrective action guidance.
