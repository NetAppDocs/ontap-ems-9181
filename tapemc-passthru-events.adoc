...
[.lead]
Learn about tapemc.passthru EMS events, which report when a data underrun (less data is transferred than requested) happens on a read operation to the device, and the number of data bytes reported as not transferred is unexpectedly larger than the number of bytes requested.
