...
[.lead]
config.move events are reported when a logical interface (LIF) migration fails in such a way that there might have been a resulting internal loss of the LIF IP address, and corrective actions typically involve following the documented corrective action guidance.
