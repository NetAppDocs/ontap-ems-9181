...
[.lead]
vsdr.vol events are reported when a Vserver DR subsystem does not enable inline deduplication on the destination volume, and corrective actions typically involve following the documented corrective action guidance.
