...
[.lead]
acp.auto events are reported when the Alternate Control Path (ACP) Administrator determines that the ACP Processor has an older firmware image compared to the file revision present in '/etc/acp/acpp_fw', and corrective actions typically involve following the documented corrective action guidance.
