...
[.lead]
Learn about acp.auto EMS events, which report when the Alternate Control Path (ACP) Administrator determines that the ACP Processor has an older firmware image compared to the file revision present in '/etc/acp/acpp_fw'.
