...
[.lead]
sis.clog events are reported when this message occurs while mounting a volume, when an error with the SIS metafile operation results in the change logging function being disabled to avoid inconsistencies, and corrective actions typically involve following the documented corrective action guidance.
