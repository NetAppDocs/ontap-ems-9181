...
[.lead]
moveagent.detected events are reported when a volume move operation is interrupted by a disaster after the critical cutover phase has started but before the source volume is deleted, and corrective actions typically involve following the documented corrective action guidance.
