...
[.lead]
raid.fdr events are reported when we detect an entry in the failed disk registry, and have forcibly failed the disk, and corrective actions typically involve following the documented corrective action guidance.
