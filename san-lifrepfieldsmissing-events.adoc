...
[.lead]
san.lifrepfieldsmissing events are reported when the system cannot replicate a SAN logical interface (LIF) in a MetroCluster(tm) configuration, because one or more attributes of the source LIF cannot be determined, and corrective actions typically involve verifying the reported configuration and health state.
