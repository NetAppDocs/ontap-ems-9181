...
[.lead]
Learn about san.lifrepfieldsmissing EMS events, which report when the system cannot replicate a SAN logical interface (LIF) in a MetroCluster(tm) configuration, because one or more attributes of the source LIF cannot be determined.
