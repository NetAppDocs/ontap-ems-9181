...
[.lead]
mlm.arraylun events are reported when data ONTAP(R) cannot install one or more discovered array LUNs since the disk device count is at full capacity, and corrective actions typically involve following the documented corrective action guidance.
