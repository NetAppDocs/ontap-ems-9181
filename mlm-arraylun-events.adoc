...
[.lead]
Learn about mlm.arraylun EMS events, which report when Data ONTAP(R) cannot install one or more discovered array LUNs since the disk device count is at full capacity.
