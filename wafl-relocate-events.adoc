...
[.lead]
wafl.relocate events are reported when the system cannot perform an aggregate relocation because one or more of its volumes are used by wafliron to host status files, and corrective actions typically involve following the documented corrective action guidance.
