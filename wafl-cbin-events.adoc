...
[.lead]
wafl.cbin events are reported when cloud mirror configuration changes during the WAFLiron operation on a FabricPool aggregate with unified format enabled, and corrective actions typically involve following the documented corrective action guidance.
