...
[.lead]
callhome.syncm events are reported when both plexes of a SyncMirror(R) configuration have disk drives in the same hardware-based disk pool; that is, the plexes are not hardware-fault isolated, and corrective actions typically involve following the documented corrective action guidance.
