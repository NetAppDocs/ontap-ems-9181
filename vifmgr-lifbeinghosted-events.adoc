...
[.lead]
vifmgr.lifbeinghosted events are reported when a logical interface (LIF) IP address that was previously marked as down because no eligible ports could host this LIF is now being hosted, and corrective actions typically involve following the documented corrective action guidance.
