...
[.lead]
vldb.mccaggregate events are reported when this message occurs during MetroCluster(tm) configuration setup, when the aggregate at the destination site is renamed to handle a name collision, and corrective actions typically involve following the documented corrective action guidance.
