...
[.lead]
max.fsa events are reported when the File System Analytics initialization scan cannot be started because the maximum number of concurrent initialization scans are already running on the cluster, and corrective actions typically involve following the documented corrective action guidance.
