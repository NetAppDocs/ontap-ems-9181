...
[.lead]
Learn about max.fsa EMS events, which report when the File System Analytics initialization scan cannot be started because the maximum number of concurrent initialization scans are already running on the cluster.
