...
[.lead]
Learn about vsdr.schedule EMS events, which report when a schedule is not available on the secondary site for replication of policies for a Vserver in a MetroCluster(tm) configuration or Vserver DR setup.
