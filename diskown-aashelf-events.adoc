...
[.lead]
diskown.aashelf events are reported when the controller cannot automatically assign a disk because disks on the shelf and disks on the stack have the same owner but belong to different pools, and corrective actions typically involve following the documented corrective action guidance.
