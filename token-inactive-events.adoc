...
[.lead]
token.inactive events are reported when a reference is deleted on a token that has existed for longer than the timeout window, and corrective actions typically involve following the documented corrective action guidance.
