...
[.lead]
ekmip.awskms events are reported when a configured Amazon Web Services (AWS) Key Management Service (KMS) that was unavailable for key operations is now available, and corrective actions typically involve following the documented corrective action guidance.
