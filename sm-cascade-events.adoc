...
[.lead]
sm.cascade events are reported when concurrent SnapMirror(R) transfers are attempted while in a cascaded scenario, which is not supported, and corrective actions typically involve following the documented corrective action guidance.
