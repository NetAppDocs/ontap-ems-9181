...
[.lead]
Learn about sm.cascade EMS events, which report when concurrent SnapMirror(R) transfers are attempted while in a cascaded scenario, which is not supported.
