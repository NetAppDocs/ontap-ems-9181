...
[.lead]
Learn about config.mixedabmodule EMS events, which report when the disk shelf stack is miscabled so that one disk shelf's module A is connected to the next disk shelf's module B, or vice versa.
