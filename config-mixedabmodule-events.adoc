...
[.lead]
config.mixedabmodule events are reported when the disk shelf stack is miscabled so that one disk shelf's module A is connected to the next disk shelf's module B, or vice versa, and corrective actions typically involve following the documented corrective action guidance.
