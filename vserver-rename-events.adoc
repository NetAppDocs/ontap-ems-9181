...
[.lead]
vserver.rename events are reported when the "vserver rename" command fails to update the peer Vserver name on the peer cluster in a Vserver peer relationship, and corrective actions typically involve following the documented corrective action guidance.
