...
[.lead]
vserver.stop events are reported when a 'vserver stop' operation fails due to failure to stop either a logical interface (LIF) or a protocol for the Vserver specified, and corrective actions typically involve ensuring the required configuration and resources are in place.
