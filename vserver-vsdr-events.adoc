...
[.lead]
vserver.vsdr events are reported when this message occurs in Async Vserver-DR setup, when an auditing-configured Vserver is starting up in Read-Only (RO) mode, and no corrective action is typically required.
