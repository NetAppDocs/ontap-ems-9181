...
[.lead]
snaplock.ss events are reported when this event is generated if an attempt is made to lock a snapshot on a SnapLock volume that has previously failed to find or load its SnapLock snapshot state, and corrective actions typically involve following the documented corrective action guidance.
