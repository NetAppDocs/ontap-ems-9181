...
[.lead]
esh.pctl events are reported when a 'power control' command cannot be performed on the specified disk device because the backplane CPLD (Complex Programmable Logic Device) is not operational, and corrective actions typically involve following the documented corrective action guidance.
