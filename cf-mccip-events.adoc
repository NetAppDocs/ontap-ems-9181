...
[.lead]
cf.mccip events are reported when automatic switchover in a MetroCluster(tm) IP configuration is disabled because the mediator mailbox disk is not healthy, and corrective actions typically involve following the documented corrective action guidance.
