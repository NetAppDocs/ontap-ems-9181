...
[.lead]
Learn about bcomd.lun EMS events, which report when the SAN Kernel Agent is unable to process a LUN metadata update resulting from a Snapshot(R) copy restore operation or other LUN event.
