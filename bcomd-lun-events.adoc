...
[.lead]
bcomd.lun events are reported when the SAN Kernel Agent is unable to process a LUN metadata update resulting from a Snapshot(R) copy restore operation or other LUN event, and corrective actions typically involve following the documented corrective action guidance.
