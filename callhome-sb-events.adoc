...
[.lead]
callhome.sb events are reported when the persistent port feature is not working during non-disruptive operations (NDO) such as takeover or giveback due to configuration errors, and corrective actions typically involve contacting NetApp technical support.
