...
[.lead]
adt.dest events are reported when the audit consolidation job fails to write audit logs to the destination directory because the destination directory is not available, and corrective actions typically involve following the documented corrective action guidance.
