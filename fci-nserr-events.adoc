...
[.lead]
fci.nserr events are reported when the Fibre Channel fabric cannot successfully complete a Name Server query issued to obtain configuration and device information when an adapter is connected to the Fibre Channel fabric, and corrective actions typically involve following the documented corrective action guidance.
