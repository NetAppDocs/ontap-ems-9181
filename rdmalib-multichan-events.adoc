...
[.lead]
rdmalib.multichan events are reported when this node's high-availability (HA) partner reports an update on the number of interconnect channels (multichan count) in use, and corrective actions typically involve following the documented corrective action guidance.
