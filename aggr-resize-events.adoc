...
[.lead]
aggr.resize events are reported when a request to resize an aggregate is sent to the REST server as part of the storage thin provisioning feature in Quark, and corrective actions typically involve following the documented corrective action guidance.
