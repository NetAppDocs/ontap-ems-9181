...
[.lead]
Learn about exports.dns EMS events, which report when a name services lookup request finds that DNS is configured as an ns-switch source for hosts, but DNS is not configured for the Vserver.
