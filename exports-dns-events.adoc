...
[.lead]
exports.dns events are reported when a name services lookup request finds that DNS is configured as an ns-switch source for hosts, but DNS is not configured for the Vserver, and corrective actions typically involve checking the reported configuration and connectivity.
