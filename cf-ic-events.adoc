...
[.lead]
cf.ic events are reported when the controller configuration is changed from a single enclosure HA configuration to a dual enclosure HA configuration, and corrective actions typically involve following the documented corrective action guidance.
