...
[.lead]
snmp.traphost events are reported when the snmp daemon could not resolve (find an IP address via DNS for) a traphost, and corrective actions typically involve following the documented corrective action guidance.
