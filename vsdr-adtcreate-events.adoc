...
[.lead]
vsdr.adtcreate events are reported when an audit staging volume cannot be created at the destination cluster in Async-DR (asynchronous disaster recovery), either because there is not enough disk space on the aggregates or the aggregates are in offline state, and corrective actions typically involve following the documented corrective action guidance.
