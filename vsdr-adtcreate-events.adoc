...
[.lead]
Learn about vsdr.adtcreate EMS events, which report when an audit staging volume cannot be created at the destination cluster in Async-DR (asynchronous disaster recovery), either because there is not enough disk space on the aggregates or the aggregates are in offline state.
