...
[.lead]
nblade.nfsmountrootonly events are reported when a non-root user performs MOUNT or NFS operation when the respective options '-mount-rootonly' or 'nfs-rootonly' are enabled, and corrective actions typically involve following the documented corrective action guidance.
