...
[.lead]
clam.partner events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveness, Availability Monitor (CLAM) is notified that its high-availability (HA) partner node is undergoing a planned shutdown, and corrective actions typically involve following the documented corrective action guidance.
