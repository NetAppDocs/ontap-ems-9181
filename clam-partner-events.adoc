...
[.lead]
Learn about clam.partner EMS events, which report when the Kernel Cluster Services (KCS) Connectivity, Liveness, Availability Monitor (CLAM) is notified that its high-availability (HA) partner node is undergoing a planned shutdown.
