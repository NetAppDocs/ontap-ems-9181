...
[.lead]
clam.invalid events are reported when the Kernel Cluster Services (KCS) Connectivity, Liveliness, Availability Monitor (CLAM) determines that the local node is in an invalid configuration for providing CLAM functionality, and corrective actions typically involve following the documented corrective action guidance.
