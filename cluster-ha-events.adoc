...
[.lead]
cluster.ha events are reported when cluster HA has not been configured on a 2-node cluster with HA mode enabled, and corrective actions typically involve following the documented corrective action guidance.
