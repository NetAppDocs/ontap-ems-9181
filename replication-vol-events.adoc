...
[.lead]
replication.vol events are reported when snapMirror(R) or VolCopy replication encounters a disk error in the active map delta blocks on the destination and cannot continue replication, and corrective actions typically involve following the documented corrective action guidance.
