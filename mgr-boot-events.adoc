...
[.lead]
mgr.boot events are reported when the system boots from its backup BIOS image because either it failed to boot from its primary BIOS image within the expected timeframe or the image is corrupted, and corrective actions typically involve following the documented corrective action guidance.
