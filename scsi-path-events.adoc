...
[.lead]
Learn about scsi.path EMS events, which report when excessive errors are encountered on this device within a short-enough time period to raise concern that there might be a faulty component between the storage initiator and device.
