...
[.lead]
replication.src events are reported when the system detects an error on a SnapMirror(R) or SnapVault(R) source volume or qtree, and corrective actions typically involve following the documented corrective action guidance.
