...
[.lead]
cpeer.psk events are reported when the system receives an authentication request for a remote cluster whose peer authentication has been revoked by the local cluster administrator, and corrective actions typically involve following the documented corrective action guidance.
