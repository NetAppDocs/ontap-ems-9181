...
[.lead]
tape.config events are reported when the tape configuration file has a format error in a required parameter or is missing a required parameter, and corrective actions typically involve following the documented corrective action guidance.
