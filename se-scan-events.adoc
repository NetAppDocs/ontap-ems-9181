...
[.lead]
se.scan events are reported when the Single Instance Storage (SIS) scanner is attempting to recompress file-based compression (secondary compression or 8k/16K/32K adaptive compression) in a volume and there is insufficient space in the volume, and corrective actions typically involve following the documented corrective action guidance.
