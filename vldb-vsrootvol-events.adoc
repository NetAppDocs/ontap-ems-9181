...
[.lead]
vldb.vsrootvol events are reported when the Vserver root volume is force deleted while non-root volumes for the Vserver still exist in the cluster, and corrective actions typically involve following the documented corrective action guidance.
