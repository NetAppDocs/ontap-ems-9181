...
[.lead]
disk.encryptaccessfwerr events are reported when a disk reports a SCSI data protection error on an attempt to download firmware and ONTAP(R) cannot restore permission to access the firmware through storage encryption methods, and corrective actions typically involve following the documented corrective action guidance.
