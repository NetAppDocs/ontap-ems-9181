...
[.lead]
copy.dst events are reported when this event is issued when a SnapMirror or aggregate copy operation attempts to update a flexible volume RAID block on the destination but is unable to do so, and corrective actions typically involve verifying the reported configuration and health state.
