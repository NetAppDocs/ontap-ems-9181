...
[.lead]
Learn about copy.dst EMS events, which report when This event is issued when a SnapMirror or aggregate copy operation attempts to update a flexible volume RAID block on the destination but is unable to do so.
