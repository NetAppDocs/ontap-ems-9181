...
[.lead]
acp.locked events are reported when the link status of a locked wrench Ethernet port (on the back of the storage controller) changes from "up" to "down", and corrective actions typically involve verifying the reported configuration and health state.
