...
[.lead]
fmmb.mmr events are reported when this event is issued when the outbound FM payload area is found to be corrupted and the master mailbox record is missing, and corrective actions typically involve following the documented corrective action guidance.
