...
[.lead]
scsi.mcc events are reported when generated when an I/O request encounters an error because of an iSCSI Protocol Error, a end device error, or some stack internal error, and corrective actions typically involve following the documented corrective action guidance.
