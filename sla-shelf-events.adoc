...
[.lead]
sla.shelf events are reported when a shelf firmware module is rebooted administratively, or when a reboot occurs normally when the system powers up, the firmware is updated, or the alternate control path (ACP) is reset, and corrective actions typically involve following the documented corrective action guidance.
