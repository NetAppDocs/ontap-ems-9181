...
[.lead]
nblade.aclcompliance events are reported when the nfs "-v4-acl-compliance" option is enabled but not functional because the SnapMirror Synchronous secondary cluster or FlexCache origin cluster is running an ONTAP version older than 9, and corrective actions typically involve following the documented corrective action guidance.
