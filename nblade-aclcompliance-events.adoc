...
[.lead]
Learn about nblade.aclcompliance EMS events, which report when the nfs "-v4-acl-compliance" option is enabled but not functional because the SnapMirror Synchronous secondary cluster or FlexCache origin cluster is running an ONTAP version older than 9.14.1.
