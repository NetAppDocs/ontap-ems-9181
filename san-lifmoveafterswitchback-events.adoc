...
[.lead]
Learn about san.lifmoveafterswitchback EMS events, which report when This message occurs during MetroCluster(tm) switchback, when a LIF cannot be placed on one of the disaster recovery (DR) nodes because it is offline.
