...
[.lead]
upgrademgr.package events are reported when a Data ONTAP(R) package is successfully imported into the cluster and processed by the upgrade manager's package handler, and corrective actions typically involve following the documented corrective action guidance.
