...
[.lead]
Learn about wafl.bagofbits EMS events, which report when a read directory request from an NFSv4 client is made to a Unicode-based directory in which directory entries with no NFS alternate name contain non-Unicode characters.
