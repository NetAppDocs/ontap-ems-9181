...
[.lead]
wafl.bagofbits events are reported when a read directory request from an NFSv4 client is made to a Unicode-based directory in which directory entries with no NFS alternate name contain non-Unicode characters, and corrective actions typically involve following the documented corrective action guidance.
